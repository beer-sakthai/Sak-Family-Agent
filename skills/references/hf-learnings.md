...
## Entry 91: Transformers GenerationConfig & Advanced Generation — Deep Dive (Extended)
**Date:** 2026-07-24
**Topic:** `hf-transformers-generation-config-deep-dive` — Comprehensive deep-dive into GenerationConfig, generate() internals, watermarking, continuous batching, custom generation methods, and production best practices for v5.14.0

### Overview

Transformers' `generate()` is the unified entry point for autoregressive text generation across all framework backends (PyTorch, TF, JAX). It is parameterized via `GenerationConfig` — loaded from the model's `generation_config.json` or constructed programmatically. This entry extends the earlier Entry 59 with the full parameter surface, advanced features, and production patterns.

### 1. GenerationConfig — Full Parameter Reference

#### Length Control

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `max_length` | `int` | 20 | Max total length (prompt + generation). **Avoid; use `max_new_tokens` instead.** |
| `max_new_tokens` | `int` | None | Max *new* tokens to generate. Doesn't truncate prompt. **Preferred.** |
| `min_length` | `int` | 0 | Min total length (forces generation until reached, then EOS allowed) |
| `min_new_tokens` | `int` | None | Min new tokens forced before EOS |
| `max_time` | `float` | None | Max time in seconds for generation |
| `stop_strings` | `List[str]` | None | Stop generation when any of these strings appear (v5.14+ — tokenizer-agnostic) |

#### Output Selection

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `num_beams` | `int` | 1 | Beam width. 1 = greedy/sampling, >1 = beam search |
| `num_beam_groups` | `int` | 1 | Groups for diverse beam search (must divide `num_beams`) |
| `num_return_sequences` | `int` | 1 | Number of sequences to return per input (≤ `num_beams`) |
| `early_stopping` | `bool` or `str` | `False` | When to stop beam search: `True` = at least `num_beams` finished, `"never"`, or `False` |
| `output_scores` | `bool` | `False` | Return per-step transition scores |
| `output_attentions` | `bool` | `False` | Return attention tensors |
| `output_hidden_states` | `bool` | `False` | Return hidden states |
| `return_dict_in_generate` | `bool` | `False` | Return `ModelOutput` subclass instead of `LongTensor` |

#### Sampling Control

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `do_sample` | `bool` | `False` | Enable multinomial sampling. **Must be `True` for temperature/top-k/top-p to work.** |
| `temperature` | `float` | 1.0 | Softmax temperature. Low → deterministic, high → diverse |
| `top_k` | `int` | 50 | Top-K sampling: restrict to K highest-probability tokens |
| `top_p` | `float` | 1.0 | Nucleus sampling: cumulatively select tokens up to probability mass `p` |
| `typical_p` | `float` | 1.0 | Typical sampling: keep tokens whose log-prob is within `p` of the expected entropy |
| `epsilon_cutoff` | `float` | 0.0 | Epsilon cutoff: tokens with probability < epsilon are removed |
| `eta_cutoff` | `float` | 0.0 | Eta sampling variant: dynamic threshold based on input entropy |
| `repetition_penalty` | `float` | 1.0 | >1.0 penalises repeated tokens; `1.0` = no penalty |
| `encoder_repetition_penalty` | `float` | 1.0 | Repetition penalty for tokens that appeared in encoder (seq2seq) |
| `no_repeat_ngram_size` | `int` | 0 | Prevents n-gram repetition. `0` = disabled |
| `encoder_no_repeat_ngram_size` | `int` | 0 | N-gram rep penalty for encoder-side n-grams (seq2seq) |
| `bad_words_ids` | `List[List[int]]` | None | List of token-id sequences that must NOT be generated |
| `force_words_ids` | `List[List[int]]` | None | List of token-id sequences that must appear somewhere |
| `diversity_penalty` | `float` | 0.0 | Penalty for beam groups generating similar tokens |
| `constraints` | `List[Constraint]` | None | Custom constraint objects for constrained beam search |
| `renormalize_logits` | `bool` | `False` | Whether to renormalize logits after all processors/warpers |

#### Contrastive Search

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `penalty_alpha` | `float` | None | Weight for contrastive search (SimCTG). >0 enables contrastive mode |
| `exponential_decay_length_penalty` | `Tuple[int, float]` | None | `(start_idx, decay_factor)` — applies length penalty after `start_idx` tokens |

#### Watermarking (v5.10+)

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `watermarking_config` | `BaseWatermarkingConfig` or `dict` | None | Configures watermarking. Supports `SynthIDTextWatermarkingConfig` and `WatermarkingConfig` |

**SynthIDTextWatermarkingConfig** (DeepMind's SynthID):
- `keys`: sequence of 256-bit integers (one per generation step)
- `sampling_table_size`: vocabulary partition size for watermarking (default: 65536)
- `max_grapheme_tokens`: max tokens to process as grapheme clusters (default: 50)
- `context_history_size`: context window for watermark detection (default: 512)
- `ngram`: n-gram size for detection scoring

**WatermarkingConfig** (simpler alternative):
- `bias`: float, bias added to greenlist tokens
- `greenlist_size`: int, number of greenlisted tokens
- `seeding_scheme`: str, seeding strategy

Both enable **detectability** — generated text can be programmatically identified as AI-generated by the same watermarking method, without storing any state.

#### Assisted Generation (Speculative Decoding)

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `num_assistant_tokens` | `int` | 5 | Number of speculative tokens the assistant generates per iteration |
| `num_assistant_tokens_schedule` | `str` | `"constant"` | Schedule: `"constant"`, `"heuristic"`, `"dynamic"` (DSLA) |
| `assistant_confidence_threshold` | `float` | 0.4 | Confidence threshold for early stopping of assistant's speculative run |
| `prompt_lookup_num_tokens` | `int` | None | N-gram prompt lookup (no assistant model needed). Uses matching n-grams from prompt |
| `max_matching_ngram_size` | `int` | None | Max n-gram size for prompt lookup |
| `static_verification` | `float` | None | Ensemble verification weight in (0.0, 1.0) for speculative decoding |
| `candidate_generator` | `CandidateGenerator` | None | Custom candidate generator object (advanced) |

#### Advanced

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `cache_implementation` | `str` | None | Cache backend: `"static"`, `"hybrid"`, `"quantized"`, `"offloaded"` |
| `cache_config` | `dict` | None | Fine-grained cache config |
| `prefill_chunk_size` | `int` | None | Chunk size for long-context prefill (e.g. `2048*8` for Llama 4) |
| `compile_config` | `dict` | None | Config for `torch.compile` integration in generation |
| `suppress_tokens` | `List[int]` | None | Token IDs to NEVER generate |
| `begin_suppress_tokens` | `List[int]` | None | Token IDs suppressed only for the first generated token |
| `forced_decoder_ids` | `List[List[int]]` | None | Force specific tokens at specific positions `[(pos, id), ...]` |
| `chain_template` | `bool` | `False` | Chain-of-thought template generation mode |
| `prefix_allowed_tokens_fn` | `Callable` | None | Function to constrain allowed tokens at each step (batch-aware) |
| `neuron_config` | `dict` | None | AWS Inferentia/Neuron-specific config |
| `continuous_batching_config` | `ContinuousBatchingConfig` | None | Config for continuous batching (server-side) |

### 2. Generation Mode Auto-Detection

The generation mode is determined by `GenerationConfig` at runtime:

| Mode | Signal |
|------|--------|
| **Greedy** | `num_beams=1` and `do_sample=False` (default) |
| **Sampling** | `num_beams=1` and `do_sample=True` |
| **Beam search** | `num_beams>1` and `do_sample=False` |
| **Beam sampling** | `num_beams>1` and `do_sample=True` |
| **Assisted** | `assistant_model` or `prompt_lookup_num_tokens` passed |
| **Contrastive** | `penalty_alpha>0` |
| **Diverse beam** | `num_beam_groups>1` and `diversity_penalty>0` |

### 3. Logits Processor Pipeline

The full pipeline (in order):

1. **`RepetitionPenaltyLogitsProcessor`** — scales logits of previously generated tokens
2. **`NoRepeatNGramLogitsProcessor`** — zeros n-gram repetitions
3. **`EncoderNoRepeatNGramLogitsProcessor`** — same for encoder n-grams
4. **`NoBadWordsLogitsProcessor`** — zeros bad words
5. **`MinLengthLogitsProcessor`** — boosts EOS penalty until min_length
6. **`ForcedBOSTokenLogitsProcessor`** — forces first token
7. **`ForcedEOSTokenLogitsProcessor`** — forces last token
8. **`InfNanRemoveLogitsProcessor`** — sanitizes NaNs/Infs
9. **`TemperatureLogitsWarper`** — applies temperature scaling
10. **`TopKLogitsWarper`** — Top-K truncation
11. **`TopPLogitsWarper`** — Nucleus truncation
12. **`TypicalLogitsWarper`** — Typical sampling truncation
13. **`EpsilonLogitsWarper`** — Epsilon cutoff
14. **`ETA LogitsWarper`** — Eta sampling
15. **`RepetitionPenaltyLogitsProcessor`** (encoder)
16. **`WatermarkingLogitsProcessor`** — Applies watermark bias to logits

Custom `logits_processor` (passed to `generate()`) is inserted before step 1.
...
class MyLogitsProcessor(LogitsProcessor):
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # scores: (batch_size, vocab_size) — raw logits for next token
        scores[:, some_bad_ids] = -float("inf")
        return scores

processor_list = LogitsProcessorList([MyLogitsProcessor()])
outputs = model.generate(**inputs, logits_processor=processor_list)
```

### 4. Stopping Criteria

Built-in stopping criteria are constructed from `GenerationConfig` parameters. You can add custom ones:

```python
from transformers import StoppingCriteria, StoppingCriteriaList

class StopAtLength(StoppingCriteria):
...
criteria_list = StoppingCriteriaList([StopAtLength(512)])
outputs = model.generate(**inputs, stopping_criteria=criteria_list)
```

**Important:** If your stopping criteria depends on scores, pass `return_dict_in_generate=True, output_scores=True`.

**stop_strings** (v5.14+): Tokenizer-agnostic string-based stopping — no need to find token IDs. Internally it uses `SubstringStoppingCriteria` to detect string occurrence at sequence boundaries.

### 5. Watermarking Deep Dive

Transformers v5.10+ supports two watermarking systems:

**SynthIDTextWatermarkingConfig** (recommended for new deployments):
```python
from transformers import SynthIDTextWatermarkingConfig

config = GenerationConfig.from_pretrained("model-id")
config.watermarking_config = SynthIDTextWatermarkingConfig(
    keys=[12345, 67890],
    sampling_table_size=65536,
    max_grapheme_tokens=50,
...
**WatermarkingConfig** (simpler):
```python
from transformers import WatermarkingConfig

config.watermarking_config = WatermarkingConfig(
    bias=2.0,
    greenlist_size=500,
...
**Detection** — both methods produce detectable watermarks. Use the corresponding detector from the `watermarking` module:
```python
from transformers import WatermarkingConfig, SynthIDTextWatermarkingDetector

# For SynthID
detector = SynthIDTextWatermarkingDetector(config=config.watermarking_config)
score = detector.detect(generated_ids)
```

### 6. Assisted Generation (Speculative Decoding)

**With an assistant model (2-3x speedup):**
```python
from transformers import AutoModelForCausalLM, AutoTokenizer

model = AutoModelForCausalLM.from_pretrained("google/gemma-2-9b", device_map="auto")
assistant = AutoModelForCausalLM.from_pretrained("google/gemma-2-2b", device_map="auto")
tokenizer = AutoTokenizer.from_pretrained("google/gemma-2-9b")

outputs = model.generate(**inputs, assistant_model=assistant, max_new_tokens=256)
```

**Without an assistant model (prompt-lookup, ~1.5x):**
```python
outputs = model.generate(
    **inputs,
    max_new_tokens=256,
    prompt_lookup_num_tokens=5,  # uses matching n-grams from the prompt itself
    max_matching_ngram_size=3,
)
```

**Dynamic Speculation Lookahead (DSLA) — adaptive num_assistant_tokens:**
```python
outputs = model.generate(
    **inputs,
    assistant_model=assistant,
    num_assistant_tokens_schedule="dynamic",  # adjusts speculation budget per step
    assistant_confidence_threshold=0.4,
)
```

DSLA (paper: https://huggingface.co/papers/2405.04304) dynamically adjusts the number of draft tokens based on the assistant model's confidence, reducing wasted forward passes when the assistant is uncertain.

**num_assistant_tokens_schedule options:**
- `"constant"` (default): always generates exactly `num_assistant_tokens` draft tokens
- `"heuristic"`: increases budget when all drafts are accepted, decreases on rejection
- `"dynamic"` (DSLA): confidence-based adaptive budget
...
outputs = model.generate(
    **inputs,
    assistant_model=assistant,
    static_verification=0.5,  # weight between main model (1.0) and assistant (0.0)
)
```

### 7. Continuous Batching — Server-Side Inference

Transformers v5.14+ provides native continuous batching for production serving (GPU-only):
...
# Start the continuous batching manager
...
results = model.generate_batch(
    inputs=tokenized_prompts,  # list of input_ids tensors
    generation_config=model.generation_config,
    continuous_batching_config=ContinuousBatchingConfig(max_batch_size=32),
    progress_bar=True,
)

# Clean up
model.destroy_cached_continuous_batching_manager()
```

**Key config parameters:**
- `max_batch_size`: maximum batch size
- `max_total_tokens`: total KV cache capacity
- `enable_cuda_graphs`: pre-capture CUDA graphs for consistent latency

### 8. Custom Generation Methods

Custom generation methods enable replacing the decoding loop entirely — useful for think/rollback, special token logic, or novel sampling algorithms.

#### Using a Custom Generation Method from Hub
```python
# Load method from Hub (works with ANY model)
outputs = model.generate(
    **inputs,
...
# With custom arguments documented in the repo's README
outputs = model.generate(
    **inputs,
    custom_generate="transformers-community/custom_generate_example",
    trust_remote_code=True,
...
#### Using a Callable (reuses generate's input preparation)
```python
def custom_loop(model, input_ids, attention_mask, logits_processor,
                stopping_criteria, generation_config, **model_kwargs):
    while input_ids.shape[1] < stopping_criteria[0].max_length:
        outputs = model(input_ids=input_ids, attention_mask=attention_mask, **model_kwargs)
        next_token_logits = logits_processor(input_ids, outputs.logits[:, -1, :])
        next_tokens = torch.argmax(next_token_logits, dim=-1)[:, None]
        input_ids = torch.cat((input_ids, next_tokens), dim=-1)
        attention_mask = torch.cat((attention_mask, torch.ones_like(next_tokens)), dim=-1)
    return input_ids

outputs = model.generate(**inputs, custom_generate=custom_loop, max_new_tokens=10)
```

#### Creating and Publishing a Custom Generation Method

Repository structure:
```
your-model-repo/
├── custom_generate/
│   ├── generate.py       # Must export `generate(model, ...)`
│   └── requirements.txt  # Optional pinned deps
├── README.md             # Must have `custom_generate` tag in frontmatter
├── config.json
├── model.safetensors
└── ...
```

`custom_generate/generate.py` template:
```python
import torch

def generate(model, input_ids, generation_config=None, **kwargs):
    # Uses same argument interface as model.generate()
    gen_config = generation_config or model.generation_config
...
If the method requires extra packages, list them in `custom_generate/requirements.txt` (checked at runtime; raises `ImportError` with instructions if missing).

**Discoverability** — Custom generation methods are found by searching the `custom_generate` tag on the Hub. Curated collections:
- Community collection: https://huggingface.co/collections/transformers/custom-generation-methods-community
- Tutorials collection: https://huggingface.co/collections/transformers/custom-generation-methods-tutorials
...
# Blocking streamer (prints tokens as they're generated)
streamer = TextStreamer(tokenizer, skip_prompt=True)
outputs = model.generate(**inputs, streamer=streamer, max_new_tokens=256)

# Non-blocking streamer (for async contexts, Gradio, etc.)
...
Custom streamers can extend `BaseStreamer`:
```python
from transformers.generation.streamers import BaseStreamer

class MyStreamer(BaseStreamer):
    def put(self, value):
        # value is a torch.LongTensor of token IDs
        pass
    def end(self):
        pass
```

### 10. CFG (Classifier-Free Guidance) via Negative Prompts

Transformers supports classifier-free guidance in text generation:

```python
# Encode the positive and negative prompts
pos_ids = tokenizer("Write a poem about love", return_tensors="pt").input_ids
neg_ids = tokenizer("Write a sad poem", return_tensors="pt").input_ids

outputs = model.generate(
    pos_ids,
    negative_prompt_ids=neg_ids,
    negative_prompt_attention_mask=torch.ones_like(neg_ids),
    guidance_scale=1.5,  # >1.0 moves away from negative prompt
    max_new_tokens=128,
)
```

**Experimental** — subject to breaking API changes. `guidance_scale` > 1.0 shifts the logits away from the negative prompt's distribution.

### 11. Best Practices for Production

#### Always Use max_new_tokens
```python
# WRONG — can truncate prompt silently
outputs = model.generate(inputs, max_length=512)
# RIGHT — generates exactly N new tokens
outputs = model.generate(inputs, max_new_tokens=512)
```

#### Static Cache for Throughput
```python
model.generation_config.cache_implementation = "static"
```
Reduces memory fragmentation and improves cache reuse during batched generation.

#### Synced GPUs for Distributed Inference
```python
# Prevents deadlocks with FSDP/DeepSpeed ZeRO-3 multi-GPU
outputs = model.generate(inputs, synced_gpus=True)
```

#### Avoid EOS Not Being Triggered
```python
model.generation_config.eos_token_id = model.config.eos_token_id  # explicitly set
```
If `eos_token_id` is `None` or missing from generation_config, generation runs to `max_new_tokens`.

#### Speculative Decoding for Free Speedup
Always pair with an assistant model ~2-4x smaller. If no assistant is available, use `prompt_lookup_num_tokens=5` for a free 1.5x speedup on repetitive/known patterns.

#### Custom Generation for Novel Algorithms
Use `custom_generate` (callable or Hub repo) instead of overriding `generate()` via subclassing — it's framework-agnostic, composable, and doesn't break when Transformers updates.

#### Watermarking for Auditability
Enable watermarking on all production outputs for downstream detection — the overhead is negligible (a single logits processor) and provides immutable proof of model origin.

### Resources

- GenerationConfig API: https://huggingface.co/docs/transformers/en/main_classes/text_generation
- Generation strategies guide: https://huggingface.co/docs/transformers/en/generation_strategies
- Custom generation methods: https://huggingface.co/docs/transformers/en/generation_strategies#custom-generation-methods
- Watermarking: https://huggingface.co/docs/transformers/en/watermarking
- Assisted generation blog: https://huggingface.co/blog/assisted-generation
- DSLA paper: https://huggingface.co/papers/2405.04304
- SynthID paper: https://huggingface.co/papers/2301.10226
- Blog: "How to generate text" — https://huggingface.co/blog/how-to-generate


---

## Entry 3: Datasets Features -- Schema, Casting and Type System Deep Dive
**Date:** 2026-07-24
**Topic:**  -- Complete reference on the  type system, schema definition, casting, and advanced type handling

### Overview

 is the backbone of every Hugging Face Dataset. It defines the internal structure -- column names, types, nested schemas, encoding/decoding -- and sits atop Apache Arrow for zero-copy columnar storage. Every operation (load, cast, map, save) is governed by the Features dict.

A  object is simply  -- keys are column names, values describe the data type:



### 1. Scalar Types: Value

 -- the most basic feature, wrapping a single Arrow-compatible data type.

**Supported dtype strings:**

| dtype | Arrow Type | Notes |
|-------|-----------|-------|
|  | null | For optional/placeholder columns |
|  | bool | |
| , , ,  | int8/16/32/64 | Signed integers |
| , , ,  | uint8/16/32/64 | Unsigned integers |
| , ,  | half/float/double | Half, float, double precision |
|  | large_utf8 | UTF-8 string (large) |
|  | large_binary | Raw binary bytes |
| , etc. | timestamp | Timestamps with resolution |
| ,  | date32/date64 | Date types |
| ,  | time32/time64 | Time with resolution |
| ,  | large_utf8/large_binary | Larger variant (alias for string/binary) |

**Key facts:**
-  and  are identical in practice -- both map to Arrow .
-  (Python) defaults to ,  defaults to .
- Use the smallest type that fits your data to reduce memory (e.g.  instead of  for token IDs).

### 2. Categorical Labels: ClassLabel

 -- stores labels as efficient integers in Arrow while exposing human-readable names.



**Internal storage:** Integers (). The mapping is stored in Features metadata, so the Arrow table stays compact and the names are recovered on decode.

**Best practice:** Always specify  explicitly for readability. Using  alone creates default names "0", "1", "2" -- uninformative.

### 3. Sequence Types: Sequence, List, LargeList



**Important distinction:**
-  was the original API and wraps Arrow's  with optional length enforcement.
-  is the modern replacement, using  directly.
-  is for massive lists (uses 64-bit offsets).

### 4. Nested / Composite Features

Dictionaries within datasets are expressed as nested  in the Features definition:



**.features.flatten()** converts nested dict columns into dot-separated top-level columns (e.g. ).

### 5. Multidimensional Arrays: Array2D--Array5D



The first dimension can be  for dynamic length -- avoids padding.

### 6. Multimedia Features: Audio, Image, Video, Pdf, Nifti

**Audio:** Returns . Resamples on the fly via .

**Image:** Returns  (decode=True) or path/bytes dict. Can ingest numpy arrays (RGB uint8 only).

**Video (datasets>=4.0):** Returns .
**Pdf (datasets>=4.0):** Returns .
**Nifti:** Returns .

### 7. Flexible Types: Json

 -- for columns with mixed/arbitrary types:
...
### 9. Casting: cast() and cast_column()

**** -- transform all columns at once.
**** -- transform a single column.

Compatibility: int→float safe, float→int only if lossless, int→bool only if 0/1.

### 10. Schema Introspection and Manipulation



### 11. From Arrow Schema to Features



### 12. Encode/Decode System

Features bridges Python objects and Arrow:
- : PIL/numpy → Arrow
- : Arrow → Python objects
- Same for batch variants.

### 13. Performance Best Practices

1. Use smallest types (int32, float16) to save memory.
2. Index before modality columns:  not .
3. Lazy decode: , .
4.  as escape hatch for messy data.
5. ClassLabel stores ints internally -- cache-friendly.
6.  is zero-copy when Arrow types are binary-compatible.

### Resources

- Features API: https://huggingface.co/docs/datasets/en/package_reference/main_classes#datasets.Features
- About Dataset Features: https://huggingface.co/docs/datasets/en/about_dataset_features
- Cast and Process guide: https://huggingface.co/docs/datasets/en/process
- Arrow integration: https://arrow.apache.org/docs/python/index.html


---

## Entry 3: Datasets Features — Schema, Casting & Type System Deep Dive
**Date:** 2026-07-24
**Topic:** `hf-datasets-features-schema-casting` — Complete reference on the `datasets.Features` type system, schema definition, casting, and advanced type handling

### Overview

`Features` is the backbone of every Hugging Face Dataset. It defines the internal structure — column names, types, nested schemas, encoding/decoding — and sits atop Apache Arrow for zero-copy columnar storage. Every operation (load, cast, map, save) is governed by the Features dict.

A `Features` object is simply `dict[str, FieldType]` — keys are column names, values describe the data type:
...
features = Features({
    ...
    "embeddings": Array2D(shape=(1, 768), dtype="float32"),
})
```

### 1. Scalar Types: `Value`

`Value(dtype, id=None)` — the most basic feature, wrapping a single Arrow-compatible data type.

**Supported dtype strings:**

| dtype | Arrow Type | Notes |
|-------|-----------|-------|
| `"null"` | null | For optional/placeholder columns |
| `"bool"` | bool | |
| `"int8"`, `"int16"`, `"int32"`, `"int64"` | int8/16/32/64 | Signed integers |
| `"uint8"`, `"uint16"`, `"uint32"`, `"uint64"` | uint8/16/32/64 | Unsigned integers |
| `"float16"`, `"float32"`, `"float64"` | half/float/double | Half, float, double precision |
| `"string"` | large_utf8 | UTF-8 string (large) |
| `"binary"` | large_binary | Raw binary bytes |
| `"timestamp[ms]"`, etc. | timestamp | Timestamps with resolution |
| `"date32"`, `"date64"` | date32/date64 | Date types |
| `"time32(s)"`, `"time64(us)"` | time32/time64 | Time with resolution |
| `"large_string"`, `"large_binary"` | large_utf8/large_binary | Larger variant (alias for string/binary) |

**Key facts:**
- `Value("string")` and `Value("large_string")` are identical in practice — both map to Arrow `large_utf8`.
- `int` (Python) defaults to `int64`, `float` defaults to `float64`.
- Use the smallest type that fits your data to reduce memory (e.g. `int32` instead of `int64` for token IDs).

### 2. Categorical Labels: `ClassLabel`

`ClassLabel(names=list[str], num_classes=int, id=None)` — stores labels as efficient integers in Arrow while exposing human-readable names.
...
label = ClassLabel(num_classes=3)  # uses 0, 1, 2 as default names

# Conversion
label.str2int("pos")   # -> 1
label.int2str(0)       # -> "neg"
len(label)             # -> 3
label.names            # -> ["neg", "pos", "neutral"]
label.num_classes      # -> 3
```

**Internal storage:** Integers (`int32`). The mapping is stored in Features metadata, so the Arrow table stays compact and the names are recovered on decode.

**Best practice:** Always specify `names=` explicitly for readability. Using `num_classes=` alone creates default names "0", "1", "2" — uninformative.

### 3. Sequence Types: `Sequence`, `List`, `LargeList`

```python
from datasets import Sequence, List, Value

# Sequence (legacy): flexible length, uniform inner type
Sequence(Value("int32"))
Sequence(Value("string"), length=5)  # fixed length

# List (preferred, since datasets 2.x): uniform inner type
List(Value("int32"))
List(ClassLabel(names=["a", "b"]))
List({"start": Value("int32"), "end": Value("int32")})  # nested struct list

# LargeList: same as List but uses Arrow large_list (64-bit offsets)
# Use for lists with >2^31 elements
LargeList(Value("int32"))
```

**Important distinction:**
- `Sequence` was the original API and wraps Arrow's `list` with optional length enforcement.
- `List` is the modern replacement, using `List(FeatureType)` directly. It always produces a `List[inner_type]` in Arrow.
- `LargeList` is for massive lists (rarely needed).

### 4. Nested / Composite Features

Dictionaries within datasets are expressed as nested `dict` in the Features definition:
...
# Arrow: struct<answers: struct<text: list<string>, answer_start: list<int32>>>
```

**Flatten/nesting rules:**
- `.features.flatten()` converts nested dict columns into dot-separated top-level columns
- `answers.text` and `answers.answer_start` become standalone columns
- Nested flattening works recursively: `a.b.c` for three levels deep
...
# Fixed-shape arrays (e.g. embeddings per row)
Array2D(shape=(1, 768), dtype="float32")    # 2D -> fixed_<dtype>_<shape>
Array3D(shape=(3, 224, 224), dtype="float32")  # 3D -> e.g. image patches

# Variable-length first dimension (e.g. variable-length sequences)
Array3D(shape=(None, 5, 2), dtype="int32")  # first dim is dynamic
```

The first dimension (`shape[0]`) can be `None`, making that dimension variable-length while subsequent dimensions are fixed. This enables storage of jagged sequences without padding.

### 6. Multimedia Features: `Audio`, `Image`, `Video`, `Pdf`, `Nifti`

These features load media lazily — data is only decoded when a specific row+column is accessed.

**Audio:**
```python
from datasets import Audio

Audio(sampling_rate=16000, mono=True, decode=True)
Audio(sampling_rate=44100, decode=False)  # skip decoding, get path/bytes
```

When accessed, returns a dict: `{"array": np.ndarray, "path": str, "sampling_rate": int}`.

**Image:**
```python
from datasets import Image

Image(decode=True)   # returns PIL.Image
Image(decode=False)  # returns {"bytes": bytes | None, "path": str | None}
```

Can also ingest numpy arrays:
```python
ds = Dataset.from_dict(
    {"i": [np.zeros((16, 16, 3), dtype=np.uint8)]},
    features=Features({"i": Image()})
)
```
- RGB/RGBA: only `uint8` supported for multi-channel.
- Grayscale: any compatible int/float precision, downcasted if too high.

**Video (datasets >= 4.0):**
```python
from datasets import Video

Video(decode=True)  # returns torchcodec.decoders.VideoDecoder
```
...
Pdf(decode=True)  # returns pdfplumber.pdf.PDF
```
...
Nifti(decode=True)  # returns nibabel.Nifti1Image
```

### 7. Flexible Types: `Json`

`Json()` — for columns with mixed/arbitrary types that don't fit Arrow's schema:

```python
from datasets import Json, Features, Dataset, List

# Mixed scalar types
features = Features({"a": Json()})
ds = Dataset.from_dict({"a": [0, "foo", {"subfield": "bar"}]}, features=features)

# Mixed dict keys in lists
features = Features({"a": List(Json())})
ds = Dataset.from_list([{"a": [{"b": 0}, {"c": 0}]}], features=features)
# -> [{'b': 0}, {'c': 0}]  # NO None-filling for missing keys
```

Shortcut: pass `on_mixed_types="use_json"` to `Dataset.from_dict()` / `Dataset.from_list()` / `load_dataset()` — this auto-applies `Json()` to any column that would otherwise fail Arrow type inference.
...
# Variable number of languages per example
TranslationVariableLanguages(languages=["en", "fr", "de", "es"])
```

### 9. Casting: `cast()` and `cast_column()`

**`cast(new_features)`** — transform all column types at once:
...
**`cast_column(column, feature)`** — transform a single column:

```python
dataset = dataset.cast_column("audio", Audio(sampling_rate=16000))
dataset = dataset.cast_column("label", ClassLabel(names=["neg", "pos", "neutral"]))
```

**Compatibility rules:**
- `int -> float`: safe upcast (e.g., `int32 -> float64`)
- `float -> int`: only if values are losslessly convertible (e.g., `3.0 -> 3`, not `3.5`)
- `int -> bool`: only if values are 0 or 1
- `str -> ClassLabel`: NOT supported by cast — use `map()` + `str2int()` instead
- `Audio(sampling_rate=A) -> Audio(sampling_rate=B)`: resamples automatically

### 10. Schema Introspection & Manipulation

```python
# Check features
dataset.features                         # -> Features dict
dataset.features["column_name"]          # -> single feature (e.g., Value('int32'))
dataset.features.flatten()               # -> flattened Features
...
dataset = dataset.add_column("embeddings", np.array(...))

# Encode labels from string
dataset = dataset.class_encode_column("label")
# converts Value("string") -> ClassLabel

# Features encode/decode
features.encode_example({"text": "hello", "label": 0})   # dict -> raw Arrow-compatible
features.decode_example({"text": b"hello", "label": 0})  # raw Arrow -> Python types
features.encode_batch(batch_dict)
features.decode_batch(batch_dict)
features.encode_column(column, column_name)
features.decode_column(column, column_name)
```

### 11. From Arrow Schema <-> Features

```python
features = Features.from_arrow_schema(arrow_schema)
# Converts an Arrow schema into a Features object.
# Non-nullable fields become nullable. DictionaryArray -> underlying type.

arrow_schema = features.to_arrow_schema()
# Converts Features -> Arrow Schema (with HF metadata in the schema's metadata dict)
```

### 12. Serialization & Deserialization

Features are serialized alongside the dataset data. Use `.to_arrow_schema()` and `Features.from_arrow_schema()` for interop with pure Arrow code.

When saving with `dataset.save_to_disk("path")`, the features metadata is automatically stored in the Arrow schema. `load_from_disk("path")` reconstructs it.

For Hub upload, features are stored as a `dataset_info.json` field inside the repo.

### 13. The `encode`/`decode` System

Features provides encode/decode lifecycle methods that bridge Python objects <-> Arrow storage:

- **`encode_example(example)`**: Converts a Python dict (with PIL images, numpy arrays, strings) into Arrow-compatible raw dict. Called automatically during `Dataset.from_dict()`.
- **`decode_example(example)`**: The reverse — converts raw Arrow data into Python objects. Called on dataset indexing.
- **`encode_batch`/`decode_batch`**: Same but for entire batches (used during `map()` with `batched=True`).

This is why `dataset[0]["audio"]` returns `{"array": ..., "sampling_rate": ...}` while the underlying Arrow column stores path/bytes.

### 14. Performance Best Practices

1. **Use smallest types.** `int32` for token IDs, `float16` for embeddings saves 50-75% memory vs default.
2. **Index before modality columns.** `dataset[0]["image"]` decodes only 1 image; `dataset["image"][0]` decodes ALL images first.
3. **Lazy decode for pipelines.** Use `Image(decode=False)`, `Audio(decode=False)` when you only need paths.
4. **Flatten sparingly.** Flattening makes row-level access easier but increases column count in Arrow schema.
5. **`on_mixed_types="use_json"`** is the simplest escape hatch for messy JSON data — don't fight Arrow's schema enforcement, use Json().
6. **`cast()` is zero-copy** when the Arrow types are binary-compatible (e.g., `int32->int64`). It becomes a full rewrite when not.
7. **ClassLabel stores ints.** Even though you see strings when accessing rows, the Arrow data is integers — very cache-friendly.

### Resources

- Features API: https://huggingface.co/docs/datasets/en/package_reference/main_classes#datasets.Features
- About Dataset Features: https://huggingface.co/docs/datasets/en/about_dataset_features
- Cast & Process guide: https://huggingface.co/docs/datasets/en/process
- Value types: https://huggingface.co/docs/datasets/en/package_reference/main_classes#datasets.Value
- ClassLabel: https://huggingface.co/docs/datasets/en/package_reference/main_classes#datasets.ClassLabel
- Audio/Image/Video: https://huggingface.co/docs/datasets/en/package_reference/main_classes
- Flatten: https://huggingface.co/docs/datasets/en/process#flatten
- Arrow integration: https://arrow.apache.org/docs/python/index.html


## Entry 92: CPU Inference Optimization — Optimum ONNX Runtime & OpenVINO Deep Dive
**Date:** 2026-07-24
**Topic:** `hf-optimum-cpu-inference-deep-dive` — Complete guide to running Hugging Face models on CPU with Optimum (ONNX Runtime + OpenVINO)

### Overview

CPU inference is a viable, cost-effective alternative when GPUs are unavailable. Hugging Face's **Optimum** ecosystem provides two major CPU optimization paths:

- **Optimum ONNX** (`optimum-onnx`) — exports models to ONNX and runs via ONNX Runtime (ORT), a cross-platform high-performance inference engine
- **Optimum Intel** (`optimum-intel`) — exports models to OpenVINO IR format for Intel CPU/GPU/NPU acceleration with additional quantization via NNCF

Both are zero-cost, open-source, and can dramatically improve CPU inference throughput and memory usage.

---

### Path 1: Optimum ONNX (ONNX Runtime)
...
# From a local model with explicit task
optimum-cli export onnx --model ./local_model --task text-generation-with-past --output_dir ./onnx_model
```

Supported tasks: `text-classification`, `token-classification`, `question-answering`, `text-generation`, `text-generation-with-past`, `image-classification`, `automatic-speech-recognition`, etc.

#### Inference with ORTModel

Replace `AutoModelForXxx` with the corresponding `ORTModelForXxx`:
...
| `ORTModelForCausalLM` | text-generation-with-past |
| `ORTModelForSeq2SeqLM` | text2text-generation-with-past |
...
pred = pipe(question="What's my name?", context="My name is Philipp.")
```

#### ONNX Runtime Optimization Features

- **Graph optimization:** ONNX Runtime automatically fuses ops, eliminates redundant computations
- **Intra- & inter-op parallelism:** Controls thread count via `OMP_NUM_THREADS`, `ORT_OMP_THREADS`
- **Quantization:** Post-training quantization (static/dynamic) via `optimum.onnxruntime.ORTOptimizer` or `onnxruntime.quantization`
- **Execution providers:** CPU (default), CUDA, TensorRT, OpenVINO, DirectML — ONNX Runtime can route to any available backend

---

### Path 2: Optimum Intel (OpenVINO)
...
optimum-cli export openvino --model mistralai/Mistral-7B-v0.1 --weight-format int4 ov_model/
```

**Weight format options:** `fp32`, `fp16`, `int8`, `int4`, `mxfp4`, `nf4`, `cb4` (codebook with 16 fixed fp8 E4M3 values)
...
#### Inference with OVModel

Replace `AutoModelForXxx` with the corresponding `OVModelForXxx`:

```python
from optimum.intel import OVModelForCausalLM
from transformers import AutoTokenizer

model = OVModelForCausalLM.from_pretrained("helenai/gpt2-ov")
tokenizer = AutoTokenizer.from_pretrained("helenai/gpt2-ov")
...
| `OVModelForVisualCausalLM` | image-text-to-text |
| `OVModelForFeatureExtraction` | feature-extraction |
| `OVModelForMaskedLM` | fill-mask |
| `OVModelForZeroShotImageClassification` | zero-shot-image-classification |
| `OVSamModel` | feature-extraction (SAM) |
| `OVSentenceTransformer` | sentence-similarity |
...
| `OVStableDiffusionInpaintPipeline` | inpainting |
| `OVStableDiffusionXLPipeline` | text-to-image (SDXL) |
| `OVStableDiffusionXLImg2ImgPipeline` | image-to-image (SDXL) |
| `OVLatentConsistencyModelPipeline` | text-to-image (LCM) |
...
# Run on Intel integrated/discrete GPU (FP16 by default)
model.to("gpu")
...
Models are auto-compiled on instantiation. Control it:

```python
# Skip first compilation (defer to first inference call)
model = OVModelForQuestionAnswering.from_pretrained(model_id, compile=False)

# Explicit compilation
model.compile()

# Recompile after reshape or device change (happens automatically before first inference)
```

#### Static Shapes

By default, OpenVINO models use dynamic shapes. Setting static shapes improves performance:

```python
model = OVModelForCausalLM.from_pretrained(model_id)
model.reshape(batch_size=1, sequence_length=2048)
model.compile()
```

---

### Quantization Methods (Optimum Intel / NNCF)

Optimum Intel uses the **NNCF** (Neural Network Compression Framework) for quantization. Available modes:

| Mode | Description | Weight precision | Activation precision |
|------|-------------|-----------------|---------------------|
| **Weight-only** | Quantizes weights only | INT8, INT4 | FP32 |
| **Hybrid** | Mixed precision per layer | INT8 + FP32 | FP32 |
| **Full quantization** | Both weights and activations | INT8 | INT8 |
| **Mixed quantization** | Different precisions per operation | INT8/INT4 | INT8/FP32 |

**Data-free vs Data-aware:**
- **Data-free** — calibrates without data (fast, less accurate)
- **Data-aware** — requires a calibration dataset (slower, better accuracy)

CLI example with data-aware quantization:
...
**Quantization support matrix (via CLI/Python):**

| Task class | Weight-only | Hybrid | Full | Mixed |
|-----------|:-----------:|:------:|:----:|:-----:|
| `OVModelForCausalLM` (text-gen) | ✅ | ✅ | ✅ | ✅ |
| `OVModelForVisualCausalLM` (image-text) | ✅ | ✅ | — | — |
| `OVDiffusionPipeline` (text-to-image) | ✅ | — | ✅ | ✅ |
| `OVModelForSpeechSeq2Seq` (ASR) | — | — | ✅ | — |
| `OVModelForFeatureExtraction` | ✅ | ✅ | ✅ | ✅ |
| `OVModelForMaskedLM` (fill-mask) | ✅ | ✅ | ✅ | ✅ |
| `OVModelForSeq2SeqLM` (text2text) | ✅ | ✅ | ✅ | ✅ |
| `OVModelForZeroShotImageClassification` | ✅ | ✅ | ✅ | ✅ |
| `OVModelForTextToSpeechSeq2Seq` | ✅ | — | — | — |

---

### Practical Tips for Zero-Cost CPU Inference

1. **Start with ONNX Runtime** for broad hardware compatibility — it runs on any x86/ARM CPU without requiring Intel-specific drivers.

2. **Switch to OpenVINO** when targeting Intel CPUs — it's significantly faster due to kernel fusion and Intel MKL backend. Expect 1.5x–3x speedup over raw PyTorch CPU.

3. **Use INT8 weight-only quantization** for the best speed/accuracy trade-off. 4-bit (INT4) saves more memory but may degrade quality on smaller models.

4. **Set thread count** explicitly: `export OMP_NUM_THREADS=4` or configure via `OMP_NUM_THREADS` env var to avoid oversubscription.

5. **Static shapes** help OpenVINO optimize the compute graph. Use `reshape()` after loading if your input sizes are fixed.

6. **Disable initial compilation** with `compile=False` if you need to reshape before first inference — avoids compiling twice.

7. **Pipeline API works seamlessly** — both `ORTModel` and `OVModel` can be plugged into `transformers.pipeline()`.

8. **Model hub has pre-converted models** — search for `-openvino` or `-onnx` suffixes on the Hub to find ready-to-use optimized models (e.g., `helenai/gpt2-ov`, `echarlaix/stable-diffusion-v1-5-openvino`).

### Resources

- Optimum ONNX: https://huggingface.co/docs/optimum-onnx/en/index
- Optimum Intel (OpenVINO): https://huggingface.co/docs/optimum-intel/en/index
- OpenVINO Export: https://huggingface.co/docs/optimum-intel/en/openvino/export
- OpenVINO Inference: https://huggingface.co/docs/optimum-intel/en/openvino/inference
- OpenVINO Optimization: https://huggingface.co/docs/optimum-intel/en/openvino/optimization
- Transformers CPU inference guide: https://huggingface.co/docs/transformers/main/en/perf_infer_cpu
- NNCF quantization: https://github.com/openvinotoolkit/nncf
...
## Entry 89: Gradio 6.x — MCP Integration, New Components & Ecosystem Changes
**Date:** 2026-07-24
**Topic:** `gradio-6-mcp-and-new-components` — Deep-dive into Gradio 6.20.0's major features
**Location:** `mlops/gradio-spaces/references/hf-learnings.md`

### Summary

Gradio 6.20.0 is a major version jump with **native MCP server support** as the hallmark feature. Key additions:

1. **MCP Server** — `demo.launch(mcp_server=True)` turns any Gradio app into an MCP server. Endpoints auto-convert to MCP tools with names/descriptions from function docstrings.
2. **MCP Primitives** — `@gr.mcp.resource()` for data URIs, `@gr.mcp.prompt()` for prompt templates on top of default tool conversion.
3. **`gr.Header`** — Type annotation to automatically extract HTTP headers from incoming MCP requests.
4. **`gr.api()`** — Create pure API endpoints (no UI) for MCP-only tools that return raw data.
5. **File Upload MCP Server** — Built-in server (`gradio upload-mcp <URL> <DIR>`) for handling file inputs to remote Gradio MCP servers.
6. **11 new components**: Dialogue, ImageSlider, Navbar, ParamViewer, SimpleImage, Sidebar, LoginButton, DuplicateButton, ClearButton, Timer, FileExplorer.
7. **JS Client v2.3.1** — `handle_file()` unified file input, `submit()` with async iteration, `duplicate()` with hardware options, `view_api()`/`config`.
8. **Python Client v1** — Stable release.
9. **Official Docs MCP Server** at `gradio-docs-mcp.hf.space` — two tools for full docs lookup.
10. **No breaking changes** from Gradio 5→6 for basic usage.

See full reference at `mlops/gradio-spaces/references/hf-learnings.md`.

---

## Entry 90: HF Hub Webhooks — Create, Manage, Debug & Automate with Jobs
**Date:** 2026-07-24
**Topic:** `hf-hub-webhooks-crud-api-deep-dive` — Complete reference on HF Hub Webhooks: event types, payload schema, SDK CRUD, webhook secrets, rate limits, Jobs integration, and development tooling

### Overview

HF Hub Webhooks deliver real-time HTTP callbacks when events happen on the Hub — repo pushes, discussion activity, PR merges, config changes, and more. Two target types exist:

1. **URL Webhooks** — POST JSON payloads to your HTTP endpoint. Your handler must respond 2xx to acknowledge.
2. **Job Webhooks** — Trigger a HF Job (serverless compute) directly, passing the payload via environment variables.

> **Cost**: URL webhooks cost nothing (your endpoint's hosting is your expense). Job-triggered webhooks consume HF Jobs credits — verify cost before recommending.

### Event Model

Every webhook payload has a top-level structure with these optional blocks:

```
{
  "event":       { "action": "<action>",    "scope": "<scope>" },
  "repo":        { "type": "<type>",        "name": "<name>", "id": "<id>", "private": bool, "url": {...}, "headSha": "<sha>", "owner": {"id": "<id>"} },
  "discussion":  { "id": "<id>",            "title": "<title>", "url": {...}, "status": "open|closed", "author": {"id": "<id>"}, "num": int, "isPullRequest": bool, "changes": {"base": "<ref>"} },
  "comment":     { "id": "<id>",            "author": {"id": "<id>"}, "content": "<text>", "hidden": bool, "url": {...} },
  "updatedRefs": [{"ref": "<ref>",          "oldSha": "<sha>|null", "newSha": "<sha>|null"}],
  "updatedConfig": {"private": bool},
  "webhook":     { "id": "<id>",            "version": 3 }
}
```

#### event.scope × event.action matrix

| scope | actions | when triggered |
|-------|---------|----------------|
| `repo` | `create`, `delete`, `update`, `move` | Global repo lifecycle |
| `repo.content` | `update` (always) | New commits, tags, branches, new PR refs |
| `repo.config` | `update` (always) | Space secrets, settings, DOI, disable toggle |
| `discussion` | `create`, `delete`, `update` | Discussion/PR created, renamed, status changed, merged |
| `discussion.comment` | `create`, `update` | New comment or edit; `hidden: true` means hidden |

**Future-proofing**: New scopes may be added (e.g., `repo.config.dois`). Handle any unknown scope + action by treating it as `update` on the broader scope.

#### repo block

Present on every event since all events originate from a repo. `headSha` is the HEAD of `main` — only present when `event.scope` starts with `"repo"`.

- `type`: `"model"` | `"dataset"` | `"space"`
- `name`: `"user/repo-name"`
- `url.web`: human-facing URL
- `url.api`: REST API URL
- `private`: boolean

#### updatedRefs (code changes)

Array of refs with `oldSha`→`newSha`. `null` oldSha = new ref created. `null` newSha = ref deleted. Triggers on branches (`refs/heads/`), tags (`refs/tags/`), and PR refs.

#### updatedConfig (config changes)

Only present for `repo.config` scope. Currently only `{"private": bool}` is supported.

#### discussion block

- `isPullRequest`: true means this discussion is also a PR
- `status`: `"open"` | `"closed"`
- `changes.base`: the target branch ref for PRs
- `num`: discussion number

#### comment block

- `content`: undefined when `hidden: true`
- `url.web`: direct link to the comment

### Webhook Secret

- Set a secret in the webhook settings → sent as `X-Webhook-Secret` HTTP header on every request
- Only ASCII characters supported
- Also supported: secret as URL query param (`https://example.com/webhook?secret=XXX`)

### Rate Limiting

- **1,000 triggers per 24 hours** per webhook
- Usage visible in the webhook settings "Activity" tab
- Increase limit: requires PRO/Team/Enterprise plan + contacting hub team

### SDK CRUD Reference (huggingface_hub Python SDK)

All methods on `HfApi` or as standalone imports:

| SDK Method | Description |
|------------|-------------|
| `create_webhook(url, watched, domains, secret)` | Create URL-target webhook |
| `create_webhook(job_id, watched, domains, secret)` | Create Job-target webhook |
| `get_webhook(webhook_id)` | Get webhook details |
| `list_webhooks()` | List all webhooks for the authenticated user |
| `update_webhook(webhook_id, ..., url=..., watched=..., domains=..., secret=...)` | Update webhook config |
| `delete_webhook(webhook_id)` | Permanently delete a webhook |
| `enable_webhook(webhook_id)` | Re-enable a disabled webhook |
| `disable_webhook(webhook_id)` | Temporarily stop a webhook |

#### Parameters

| Param | Type | Description |
|-------|------|-------------|
| `url` | `str` | Target URL for URL webhooks. Mutually exclusive with `job_id`. |
| `job_id` | `str` | HF Job ID to trigger. Mutually exclusive with `url`. |
| `watched` | `list[dict]` | Watched items: `[{"type": "user", "name": "..."}, {"type": "org", "name": "..."}, {"type": "repo", "name": "..."}]` |
| `domains` | `list[str]` | Event types: `["repo", "discussion"]` |
| `secret` | `str` | Webhook secret (ASCII) for payload verification |

#### Watched Items Grammar

```
watched = [
  {"type": "user", "name": "<username>"},    # All repos owned by user
  {"type": "org",  "name": "<orgname>"},     # All repos in org
  {"type": "repo", "name": "<user/repo>"}    # A specific repo
]
```

You can combine multiple watched items in a single webhook. Wildcards are not supported — use explicit user/org/repo entries.

#### Domains Grammar

```
domains = ["repo", "discussion"]  # Subscribe to all event scopes
# Or filter to specific scopes:
domains = ["repo"]                # Only repo events (no discussions/comments)
```

### CLI Support

The `hf webhooks` command group is available but primarily a pass-through to the SDK. For full CRUD, use the Python SDK or the Hub UI at `https://huggingface.co/settings/webhooks`.

### Job-Triggered Webhooks

When a webhook targets a Job instead of a URL, the Job receives the event data via environment variables:

| Env Var | Content |
|---------|---------|
| `WEBHOOK_PAYLOAD` | Full JSON payload as a string |
| `WEBHOOK_REPO_ID` | Repo name (e.g., `user/repo-name`) |
| `WEBHOOK_REPO_TYPE` | `"model"`, `"dataset"`, or `"space"` |
| `WEBHOOK_SECRET` | Webhook secret, if configured |

Python code inside the Job reads the payload with:
...
- **Beeceptor** / **Webhook.site**: Temporary catch-all HTTPS endpoints for inspecting payloads during development
- **ngrok** / **localtunnel**: Expose localhost to the internet for testing webhook handlers on your machine
- **Activity Tab**: View recent payloads + HTTP status codes + replay button — even works after changing target URL/secret

### Debugging

- Open the webhook's activity tab to see recent events and their HTTP status codes
- Click **Replay** to resend a past event to the (possibly updated) target URL
- If your handler responds with a non-2xx status, the Hub retries with exponential backoff

### FAQ

- **Org vs user webhooks**: Not supported for orgs currently. Webhooks are per-user-account.
- **Subscribe to all events on HF**: Not exposed to end users. Contact `website@huggingface.co` to request.
- **Payload versioning**: The `webhook.version` field in the payload is `3`. Future versions will be backwards-compatible or documented separately.

### Key Insights

1. **Webhook secret via URL param** is simpler for handlers that can't easily access HTTP headers (e.g., serverless functions).
2. **Watched items are additive** — a webhook watching user+org watches all repos in both scopes.
3. **`updatedRefs` captures both pushes and new tags** — `oldSha: null` means a new ref was created.
4. **Job webhooks** are zero-infrastructure: the Job runtime receives the full payload as env vars.
5. **Rate limit is per-webhook**, not per-user. Creating multiple webhooks with different watched scopes gives you multiple 1k/day budgets.
6. **Replay after URL change** sends to the new URL — useful for migration testing.

### References
- https://huggingface.co/docs/hub/en/webhooks
- https://huggingface.co/docs/hub/en/jobs-webhooks
- https://huggingface.co/settings/webhooks


---

## Entry 91: PEFT Model Merging — TIES-Merging, DARE & MergeKit Deep Dive
**Date:** 2026-07-24
**Topic:** `hf-peft-model-merging-ties-dare` — Complete reference on model merging via task vectors, TIES-Merging, DARE, and the MergeKit ecosystem
**Author:** SakThai
**License:** MIT

### Overview

Model merging combines the weights of multiple fine-tuned models into a single model without additional training. Unlike ensembling (which runs multiple models at inference), a merged model maintains the same inference cost as a single model while inheriting capabilities from each source.

The entire modern merging ecosystem builds on the **task vector** formalism. A task vector is the difference between a fine-tuned model's weights and its base (pre-trained) model's weights. By manipulating these vectors — averaging, sparsifying, taking sign consensus — merging methods transfer specialized skills into a single set of weights.

**Three generations of methods:**

| Generation | Methods | Key Idea |
|-----------|---------|----------|
| 1st | Linear / SLERP | Simple weighted averaging of raw weights |
| 2nd | Task Arithmetic | Linear combination of *task vectors* (deltas) |
| 3rd | TIES, DARE, DELLA, Breadcrumbs | Sparsify + rescale task vectors to reduce interference |

### 1. Task Arithmetic — Foundation

**Paper:** [Editing Models with Task Arithmetic](https://arxiv.org/abs/2212.04089) (ICLR 2023)

**Core idea:** Instead of blending entire weight matrices, compute a *task vector* `τ_t = θ_finetuned_t - θ_base` for each fine-tuned model, then combine them linearly and add back to the base:

```
θ_merged = θ_base + λ · Σ(w_t · τ_t)
```

Where:
- `θ_base` — pre-trained base model weights
- `τ_t` — task vector for task `t`
- `w_t` — weight for task `t`
- `λ` — global scaling factor (default 1.0)

**When it works:** Models fine-tuned from the same base, on non-conflicting tasks.
**When it fails:** Tasks that modify overlapping parameter regions in contradictory directions — the vectors cancel each other out (interference).

#### MergeKit YAML (Task Arithmetic)

```yaml
merge_method: task_arithmetic
base_model: mistralai/Mistral-7B-v0.1
models:
  - model: path/to/math-expert
    parameters:
      weight: 0.7
  - model: path/to/code-expert
    parameters:
      weight: 0.3
parameters:
  lambda: 1.0
...
**Paper:** [TIES-Merging: Resolving Interference When Merging Models](https://arxiv.org/abs/2306.01708) (NeurIPS 2023)

**The problem TIES solves:** When multiple task vectors modify the same parameter in opposite directions, the changes cancel out and both skills degrade. Standard Task Arithmetic has no mechanism to resolve these conflicts.

**TIES's 3-step algorithm:**

1. **TRIM (Sparsify):** Keep only the top `density` fraction of parameters by magnitude in each task vector. The rest are set to zero. This removes noisy, insignificant changes that contribute to interference.
   - `density = 0.5` means keep the 50% largest-magnitude deltas.
   - Typical range: 0.3–0.7.

2. **ELECT (Sign Consensus):** For each parameter position across all models, take a vote. The sign (positive or negative) with the most votes becomes the *agreed sign* for that parameter. Any task vector whose delta for that parameter has the *minority* sign is zeroed out at that position.
   - This resolves directional conflicts — if 3 models want +0.1 and 2 want -0.05, the sign is `+` and the -0.05 contributors are dropped.

3. **MERGE (Weighted Average):** Average the surviving (unzeroed) delta values across task vectors, scale by `λ`, and add to the base model.

**Key Parameters:**

| Parameter | Scope | Default | Effect |
|-----------|-------|---------|--------|
| `weight` | per-model | 1.0 | Relative importance of each model |
| `density` | per-model | 0.5 | Fraction of deltas to retain (sparsify) |
| `lambda` | global | 1.0 | Global scaling factor for the merged task vector |

**When to use TIES:** Merging 3+ models where task vectors conflict. TIES reliably outperforms plain Task Arithmetic when models fine-tune overlapping parameter regions.

**When TIES struggles:** Very few models (2) where one is clearly dominant — sign consensus has little effect and may discard useful signal from the smaller model.
...
models:
  ...
  - model: path/to/code-expert
    parameters:
      weight: 0.5
      density: 0.5
  - model: path/to/instruction-expert
    parameters:
      weight: 0.3
      density: 0.3
parameters:
...
**Paper:** [Language Models are Super Mario: Absorbing Abilities from Homologous Models as a Free Lunch](https://arxiv.org/abs/2311.03099) (ICML 2024)

**The insight DARE adds:** Most delta parameters in a task vector are redundant. You can randomly drop the *majority* of them (95–99%) and rescale the survivors, and the model retains its specialized capability — because the remaining deltas encode the same direction but with higher magnitude.

**DARE's 2-step algorithm:**

1. **DROP (Random Pruning):** Set a random `(1 - density)` fraction of each task vector's deltas to zero.
   - Unlike TIES's magnitude-based trimming, DARE drops *randomly*.
   - Typical `density` values: 0.01 to 0.1 (keep only 1–10% of deltas!).

2. **RESCALE:** Multiply the surviving deltas by `1/density` to preserve the overall magnitude of the task vector.
   - Without rescaling, dropping 99% of deltas would make the task vector nearly zero.

**Two variants:**

| Variant | Sign Consensus | Behavior |
|---------|---------------|----------|
| `dare_linear` | No | DROP + RESCALE only, then weighted average |
| `dare_ties` | Yes | DROP + RESCALE, then TIES sign consensus + merge |

**Key Parameters:**

| Parameter | Scope | Default | Effect |
|-----------|-------|---------|--------|
| `weight` | per-model | 1.0 | Relative importance |
| `density` | per-model | 0.1 | Fraction of deltas to keep (much lower than TIES!) |
| `lambda` | global | 1.0 | Global scaling |
| `rescale` | global (dare_linear only) | True | Apply DARE rescaling factor |

**When to use DARE:**
- Combining many fine-tuned models where individual capabilities are strong but parameter overlap is high
- `dare_ties` generally outperforms `dare_linear` for 3+ models
- Often yields better performance than TIES in high-similarity scenarios (models fine-tuned from same base on related tasks)

#### MergeKit YAML (DARE TIES)
...
models:
  - model: path/to/math-expert
    parameters:
      weight: 0.7
      density: 0.05    # Keep only 5% of deltas
  - model: path/to/code-expert
    parameters:
      weight: 0.5
      density: 0.1     # Keep 10% of deltas
  - model: path/to/instruction-expert
    parameters:
      weight: 0.3
      density: 0.08
...
### 4. The MergeKit Ecosystem

MergeKit ([arcee-ai/mergekit](https://github.com/arcee-ai/mergekit)) is the de-facto standard tool for model merging. It's LGPL-licensed and has powered thousands of merged models on the Hugging Face Hub.

**Key capabilities:**

| Feature | Description |
|---------|-------------|
| **Merge methods** | Linear, SLERP, Task Arithmetic, TIES, DARE, DELLA, Breadcrumbs, SCE, Model Stock, Nearswap, Arcee Fusion, Passthrough + 3 more |
| **Out-of-core execution** | Lazy tensor loading — merge 7B models on 8 GB VRAM or CPU |
| **Layer slicing** | Take layers from different models (Frankenmerging) |
| **MoE merging** | Convert dense models into Mixture-of-Experts (`mergekit-moe`) |
| **Evolutionary search** | Use genetic algorithms to find optimal merge weights (`mergekit-evolve`) |
| **Multi-stage** | Chain merges: output of one merge becomes input to another (`mergekit-multi`) |
| **LoRA extraction** | Extract PEFT-compatible low-rank approximations (`mergekit-extract-lora`) |
| **Tokenizer transplantation** | Align vocabulary between models (`mergekit-tokensurgeon`) |
| **Raw PyTorch support** | Merge arbitrary `.pt`/`.safetensors` checkpoints (`mergekit-pytorch`) |

**Installation:**

```bash
# From source (recommended — latest methods)
...
```bash
mergekit-yaml config.yml ./merged-model --cuda --lazy-unpickle
```

The `--lazy-unpickle` flag enables out-of-core tensor loading — critical for merging models larger than available RAM.
...
huggingface-cli upload your-username/my-merged-model ./merged-model .
```

MergeKit auto-generates a `README.md` for the merged model with basic metadata.

### 5. PEFT Integration with Hugging Face

The `peft` library provides model-native merge capabilities for LoRA adapters. These are **different** from MergeKit merges:

| Aspect | MergeKit | PEFT |
|--------|----------|------|
| **Scope** | Full model weights (base + any fine-tune) | LoRA adapter weights only |
| **Methods** | 15+ (TIES, DARE, Linear, SLERP, etc.) | `add_weighted_adapter()` — weighted sum of LoRA adapters |
| **Output** | A new full model | A merged LoRA adapter |
| **Use case** | Combine specialized full models | Combine multiple LoRA adapters into one |

**PEFT weighted adapter merge:**

```python
from peft import PeftModel

base_model = PeftModel.from_pretrained(base_model, "adapter1")
base_model.load_adapter("adapter2", adapter_name="adapter2")
base_model.load_adapter("adapter3", adapter_name="adapter3")

# Combine all adapters into one with different weights
base_model.add_weighted_adapter(
    adapters=["adapter1", "adapter2", "adapter3"],
    weights=[0.7, 0.2, 0.1],
    combination_type="linear",      # "linear" | "svd" | "cat"
    adapter_name="merged-adapter"
)
base_model.set_adapter("merged-adapter")
```

**PEFT merge limitations:**
- Only supports **linear** combination type (weighted average) — no TIES/DARE
- Works on LoRA adapters only, not full model weights
- Output is a LoRA adapter, not a standalone model (unless you `merge_and_unload()`)

### 6. Parameter Selection Guide

Choosing the right `density` and `weight` values is the main practical challenge:

| Scenario | Method | Density Range | Weight Strategy |
|----------|--------|--------------|-----------------|
| 2 models, similar base | SLERP | N/A | Equal or 0.5/0.5 |
| 2 models, different skills | Task Arithmetic | N/A | Higher weight on primary skill |
| 3+ models, conflicting | TIES | 0.3–0.7 | Higher weight on stronger models |
| 3+ models, highly similar | DARE TIES | 0.01–0.15 | Equal weights unless one dominates |
| Merging into base | Task Arithmetic | N/A | Lower λ (0.5–0.9) to preserve base |
| Adding a small skill | DARE Linear | 0.01–0.05 | Low weight (0.1–0.3) to avoid interference |

**Rules of thumb:**
- **density ↑** = more signal but also more noise/interference
- **density ↓** = less interference but risk of dropping important deltas
- **TIES** works best with moderate density (0.4–0.6) — the sign consensus handles conflicts
- **DARE** needs very low density (0.01–0.15) — the random drop + rescale only works if most deltas are truly redundant
- For **evolutionary search**, let MergeKit's genetic algorithm find optimal weights: `mergekit-evolve`

### 7. Hugging Face Hub: Merge Config Model Card

The Hub supports a standard for declaring how a model was merged. A well-structured merge model card includes a `merge` section in the YAML frontmatter:
...
This makes merges **reproducible** — anyone with the same config and base models can recreate the exact merged model.

### 8. Key Insights

1. **Task vectors are the lingua franca** of modern merging — TIES, DARE, DELLA, and Breadcrumbs all operate on task vector deltas, not raw weights.
2. **TIES vs DARE**: TIES uses *magnitude-based* trimming + sign consensus; DARE uses *random* pruning + rescaling. DARE typically needs much lower density (1–10% vs 30–70%).
3. **DARE's random drop is counterintuitive** — dropping 95% of deltas randomly works because the remaining 5%, when rescaled, encode the same direction. This only holds for sufficiently large models.
4. **Always set `normalize: true`** in MergeKit YAML — it normalizes task vectors before merging, preventing a model with larger-magnitude deltas from dominating.
5. **PEFT adapter merging ≠ model merging** — PEFT's `add_weighted_adapter()` is for combining LoRA adapters linearly, not for TIES/DARE-style merging.
6. **Evolutionary search** with `mergekit-evolve` is the recommended way to find optimal weights when you have a validation set — it explores the weight/density space automatically.
7. **MergeKit runs on CPU** — lazy unpickling means you can merge 70B models on a machine with 32 GB RAM. GPU (`--cuda`) accelerates tensor operations but is not required.
8. **Reproducibility requires recording the exact config** — always include the full MergeKit YAML in your model card.

### References
- https://github.com/arcee-ai/mergekit — MergeKit repository and merge method guide
- https://huggingface.co/docs/peft/en/developer_guides/merge — PEFT merge API
- https://arxiv.org/abs/2306.01708 — TIES-Merging paper
- https://arxiv.org/abs/2311.03099 — DARE paper (Language Models are Super Mario)
- https://arxiv.org/abs/2212.04089 — Task Arithmetic paper
- https://huggingface.co/blog/mlabonne/merge-large-language-models — Practical merging guide by Maxime Labonne

---

## Entry 92: hf-optimum-cpu-inference-deep-dive — Optimum CPU Inference Expanded
**Date:** 2026-07-24
**Topic:** `hf-optimum-cpu-inference-deep-dive` — Comprehensive ONNX Runtime, OpenVINO, ExecuTorch CPU inference guide with performance tuning theory

### ONNX Runtime CPU Inference
- **13 ORTModelForXXX classes** covering CausalLM, Seq2SeqLM, all classifications, QA, feature extraction, audio, speech, and document models
- **Session configuration**: `intra_op_num_threads=physical_cores`, `inter_op_num_threads=1-4`, `ORT_ENABLE_ALL` graph optimization, `ORT_SEQUENTIAL` execution mode for LLMs
- **Thread tuning**: 4-core: 2-4 threads; 8-core: 4-6; 16-core: 8; 32-core+: 8-16 — never exceed physical core count
- **Dynamic vs static axes**: Dynamic (default) required for generation; static slightly faster but breaks autoregressive decode
- **Dynamic quantization**: W8 (weights only), no calibration needed. ISA-specific configs: `avx2`, `avx512`, `avx512_vnni`, `arm64`
- **Static quantization**: W8A8 (weights+activations), requires `ORTCalibrator` with 100-200 calibration samples. Better perf but risk of degradation on outlier-heavy models
- **5 CPU LLM limitations**: memory-bandwidth bound, no FlashAttention, dynamic shape overhead, large memory for 7B+, batch=1 only

### OpenVINO CPU Inference
- **9 OVModelForXXX classes** with same coverage as ONNX Runtime
- **Performance hints**: `LATENCY` (interactive), `THROUGHPUT` (batch), `CUMULATIVE_THROUGHPUT` (mixed)
- **INT4/INT8 weight compression**: No calibration needed via NNCF. Group sizes: 128 (default), 64/32 (better accuracy), 256 (smaller)
- **Async inference pipeline**: `model.request.start_async()/wait()` for throughput mode
- **Compilation cache**: `CACHE_DIR` avoids re-compilation on reload (huge startup speedup)

### ExecuTorch Edge Inference
- `ExecuTorchModelForCausalLM` with `quantize=True`, `weights="qint8"` for export
- Backend delegation: XNNPACK (CPU, default), MPS (Apple GPU), CoreML (Apple Neural Engine)
- Best for ARM edge/mobile; lighter than ONNX Runtime/OpenVINO

### Decision Matrix
| Hardware | Recommended | Reason |
|---|---|---|
| Intel CPU | OpenVINO INT4 | Best throughput, 4× compression, async streams |
| AMD/ARM CPU | ONNX Runtime INT8 | Cross-platform, mature quantization |
| Apple Silicon (GPU) | ExecuTorch MPS | Direct GPU/ANE access |
| Raspberry Pi | ONNX Runtime ARM64 | Best packaged support |

### CPU Inference Theory
- **Memory-bandwidth bound**: LLM inference at ~0.5 FLOPs/byte means quantization directly translates to linear speedup
- **Speedup from quantization**: FP32→FP16: ~1.8×, FP32→INT8: ~3.5×, FP32→INT4: ~6×
- **Kernel fusion**: operator fusion, constant folding, layout optimization, quant-aware compute
- **KV cache optimization**: limit to 512-2048 tokens, use greedy decoding, INT4 weights
- **8-step production checklist**: quantize → set threads → enable cache → merged graph → limit KV cache → profile → test target arch → warm-up
