...
## 2026-09-04 - Relative System-Root Path Validation Gap in Workflow Executor
**Vulnerability:** `_validate_filepath` in `executor.py` validated absolute paths against `system_roots` (`/etc`, `/bin`, `/var`, etc.), but relative paths targeting system roots without leading slashes (e.g., `etc/passwd`, `var/log/syslog`, `usr/bin/bash`) bypassed root directory checks prior to resolution.
**Learning:** Filepath validation helpers that protect system roots must check relative path components before resolving targets against the working directory, as relative paths whose leading component matches a system root can access system files when executed from `/` or relative locations.
**Prevention:** Inspect `Path(normalized_str).parts` on relative paths before resolution and reject any relative reference whose first component matches `system_roots` (with a single-component `tmp` exception for local usage).
