...
## 2026-09-04 - Process Substitution Execution Bypass via Command Wrappers
**Vulnerability:** Shell command validation in the Workflow Executor (`_validate_shell_command`) detected direct process substitution invocations (e.g. `bash <(...)` or `tee >(bash)`), but checked only the raw first token of outer and inner process substitution expressions. Attackers could bypass process substitution restrictions by wrapping target shell interpreters or scripts in transparent wrappers (e.g., `tee >(env bash)`, `tee >(sudo python3)`, or `env bash <(...)`).
**Learning:** Checking only the raw un-unwrapped first token when evaluating process substitution commands allows wrapped interpreters to bypass security checks.
**Prevention:** Implement a transparent wrapper unwrapping helper (`_get_effective_command`) that strips environment variable assignments, flags, and common wrappers (`env`, `sudo`, `timeout`, `exec`, etc.) to resolve the true underlying executable before validating process substitutions.
