...
## 2026-09-07 - Double/Multi-Layered URL Encoded relative path traversal in Static Server & Executor
**Vulnerability:** Static file handlers (`personas/*/web/server.py` and `scripts/serve_api.py`) and workflow executor (`apps/agent_workflow_framework/agent_workflow/executor.py`) only decoded URL-encoded input paths once (`unquote(parsed.path)`). Downstream handlers or `SimpleHTTPRequestHandler` unquote paths again, allowing attackers to bypass path traversal checks (`..`) using multi-layered URL encoding (`%252e%252e`).
**Learning:** Single-pass URL decoding is insufficient when downstream framework components or stdlib handlers perform additional URL unquoting.
**Prevention:** Always recursively unquote/URL-decode user-supplied paths up to a safe maximum depth (e.g. 5 passes with early exit) before checking for relative traversal segments (`..`) or verifying static root directory containment.

## 2026-09-08 - SSRF Fragment and Backslash Authority Confusion Bypasses
**Vulnerability:** The HTTP fetch action in the workflow executor was vulnerable to Server-Side Request Forgery (SSRF) bypasses. Attackers could supply URLs with fragment identifiers (e.g., `http://127.0.0.1#@example.com`) or backslashes in the authority component (`http://example.com\@127.0.0.1`). Naive URL parsing and IP resolution might process these inputs differently than underlying HTTP clients, allowing an attacker to coerce the server into accessing prohibited internal services. Furthermore, Python's `ipaddress` module does not automatically unwrap IPv4-mapped IPv6 addresses (e.g., `::ffff:127.0.0.1`), meaning `is_global` checks would fail to block private IPv4 endpoints disguised as IPv6.
**Learning:** SSRF protections must closely mimic the parsing quirks of the actual networking client executing the request. Stripping fragments prior to parsing, rejecting backslashes in network locations, and specifically unwrapping mapped IP addresses are necessary defense-in-depth measures when relying on standard library URL parsers.
**Prevention:** Implement comprehensive URL validation helpers that explicitly strip URL fragments (`#`), reject backslashes (`\`) in `netloc` components, and unwrap IPv4-mapped IPv6 structures (`ip.ipv4_mapped`) before enforcing non-public/loopback routing constraints.
