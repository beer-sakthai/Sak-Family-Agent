...
## 2026-09-04 - Process Substitution Outer Interpreter Bypasses via Wrappers and Environment Variables
**Vulnerability:** The Workflow Executor's shell command validation (`_validate_shell_command`) checked `parts[0]` directly to detect outer interpreters taking process substitution parameters (e.g., `bash <(...)`), but did not unwrap transparent wrappers, environment variables, or flags. An attacker or workflow could bypass process substitution restrictions by wrapping the outer interpreter (e.g., `env bash <(...)`, `sudo python3 <(...)`, `timeout 10 zsh <(...)`, `exec sh <(...)`).
**Learning:** Outer command inspection in security validators must unwrap CLI wrappers, environment variables, and flags before checking if the underlying executable is a restricted interpreter. Inspecting only `parts[0]` allows wrapped execution contexts to hide restricted outer commands.
**Prevention:** Iteratively unwrap `parts` through known wrapper binaries, environment variable assignments, and flags to identify the true underlying `cmd_word` before evaluating outer command security rules.
