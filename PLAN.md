...
| **Test coverage audit follow-on (analysis only)** — re-measured on `main` at `a704a40` with the 2026-08-26 audit's exact command (2,251 passed / 2 skipped / 6 deselected / 124 subtests, 106 files): **95.849% branch coverage against `fail_under = 96` — the floor is breached** and `pytest` exits 1 locally. In one day the suite gained 247 tests while coverage fell 0.169pp (uncovered statements 168 → 213, partials 149 → 170), which is exactly the undetectable erosion the previous audit's finding 8 predicted at 0.018pp of headroom. Six findings in [`docs/test-coverage-audit-2026-08-27.md`](./docs/test-coverage-audit-2026-08-27.md). New: the gate is a package-wide average with no per-file or diff floor, so new code can only dilute it; the `client/` onboarding subsystem merged today in #1198 (`cli/client.py` 87.6%, `client/manager.py` 89.5%, `client/verifier.py` 88.3%, `client/models.py` 93.4%) contributes 45 uncovered units, all of it error/exit and interactive-prompt paths; `sakthai-chat-cli/`'s 72 test files are run by no workflow (only bandit/codeql scan the tree); and the coverage number moves depending on whether the suite runs as root, which is the likeliest reason CI passes where a local root run fails. Carried and re-verified unchanged: `guardrails.py` 89% with the container rules at 866–910 unreachable behind `_check_destructive_tokens`, `telegram/bot.py` hidden by `omit`, the web-auth rejection paths, `team/engine.py` parallel failures. Every package file except the omitted `telegram/bot.py` is measured — no dark modules. No production code changed | ✅ Done (2026-08-27) |
