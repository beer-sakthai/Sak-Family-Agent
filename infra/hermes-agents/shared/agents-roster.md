...
**SakSit**, **SakTan**, and **SakJules**. All six are Telegram agents owned by Beer (`beer-sakthai`).
...
> **SakSit**, `saktan` hosts **SakTan**, `sakjules` hosts **SakJules**. The lead, **SakKing**, lives on the
...
- Role: **GitHub Repository Steward** — code, issues, pull requests, Actions, Projects, Security, Insights, Settings.
- Runtime: Hermes gateway, **sakjules profile** (`HERMES_HOME=/home/sakthai/.hermes/profiles/sakjules`).
- Model: **Google Gemini** — `gemini-3.1-pro`, with a **Gemini fallback chain**: `gemini-3-flash-preview` → `gemini-3.1-flash-lite` → `gemini-3.5-flash` → `gemini-2.5-flash-preview-native-audio-dialog`.
