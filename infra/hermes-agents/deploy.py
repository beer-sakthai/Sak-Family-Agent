...
def main():
    ...
    print(f"Deploying configurations...")
    ...
    if os.path.exists(systemd_src):
        ...
        for root, dirs, files in os.walk(systemd_src):
            for file in files:
    ...
    try:
        ...
        res = subprocess.run(['systemctl', '--user', 'daemon-reload'], capture_output=True, text=True, check=True)
