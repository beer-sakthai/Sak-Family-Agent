...
class MemoryStore:
    ...
    The connection is opened with ``check_same_thread=False`` because the TUI
    runs the agent loop (and therefore this store's methods) on a Textual
    worker thread while the connection itself is created on the main thread.
    A single connection touched from multiple threads is only safe if calls
    are serialised, so every public method that executes SQL takes
    ``self._lock`` (an ``RLock`` so methods that call other public methods,
    e.g. :meth:`render_prompt_block`, don't deadlock on themselves).
    """
