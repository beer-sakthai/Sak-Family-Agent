...
def test_init_with_different_iterable_types() -> None:
    t1 = _tool("t1")
    t2 = _tool("t2")

    # Tuple
    reg_tuple = ToolRegistry((t1, t2))
    assert reg_tuple.names() == ["t1", "t2"]

    # Generator expression
    reg_gen = ToolRegistry(t for t in [t1, t2])
    assert reg_gen.names() == ["t1", "t2"]
    ...
    reg_custom = ToolRegistry(CustomIterable())
    assert reg_custom.names() == ["t1", "t2"]


def test_contains_non_string_types() -> None:
    reg = ToolRegistry([_tool("t1")])
    assert 123 not in reg
    assert None not in reg
    assert object() not in reg


def test_tools_property_immutability_and_isolation() -> None:
    t1 = _tool("t1")
    reg = ToolRegistry([t1])
    tools_tuple = reg.tools
    assert isinstance(tools_tuple, tuple)
    assert tools_tuple == (t1,)
    # Verify that calling .tools returns a new tuple instance
    assert reg.tools is not reg.tools


def test_init_duplicate_preserves_initial_key_order() -> None:
    t1_old = _tool("a", "v1")
    t2 = _tool("b", "v1")
    t1_new = _tool("a", "v2")

    reg = ToolRegistry([t1_old, t2, t1_new])
