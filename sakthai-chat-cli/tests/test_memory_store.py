...
def test_store_is_usable_from_a_different_thread(store: MemoryStore) -> None:
    """Regression for C1 (final-review finding).

    ``store`` (the fixture) opens a file-backed MemoryStore on the main/pytest
    thread, exactly as ``sakthai/cli/chat.py`` does. The Textual chat TUI then
    calls store methods (``render_prompt_block()``, tool/guardrail ops) from a
    worker THREAD (``sakthai/agent/tui.py``'s ``run_agent`` worker). Without
    ``check_same_thread=False`` on the connection, any store call from that
    other thread raised ``sqlite3.ProgrammingError``. This exercises the store
    seam directly: write a fact from a background thread, then read it back
    from the main thread through the same connection.
    """
    errors: list[BaseException] = []

    def _worker() -> None:
        try:
            store.add_fact("written from a worker thread", kind="note", key="cross-thread")
            # Also exercise render_prompt_block(), the call every real chat
            # turn makes while building the system prompt.
            store.render_prompt_block()
        except BaseException as exc:  # noqa: BLE001 - hand failures back to the main thread
            errors.append(exc)
    ...
    assert not thread.is_alive(), "store call did not finish inside the worker thread"
    assert errors == [], f"store call raised inside the worker thread: {errors!r}"

    # Written on the worker thread, read back on the main thread.
    fact = store.get_fact_by_key("note", "cross-thread")
    assert fact is not None
    assert fact.value == "written from a worker thread"
...
def test_deduplicate_facts_security_sql_injection_payload(store: MemoryStore) -> None:
    ...
    # Even if they have injection strings, our secure parameterization makes injection impossible
