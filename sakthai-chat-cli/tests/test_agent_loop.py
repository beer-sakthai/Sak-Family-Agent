...
def test_run_agent_from_worker_thread_does_not_raise_programming_error(
    store: MemoryStore,
) -> None:
    """Regression for C1 (final-review finding).

    The Textual chat TUI opens ``MemoryStore`` on the main thread
    (``sakthai/cli/chat.py``) but runs ``run_agent`` inside a worker THREAD
    (``sakthai/agent/tui.py``); the agent loop always touches the store, at
    minimum via ``render_prompt_block()`` while building the system prompt,
    and here also via the ``learn`` tool's ``add_fact()``. Without
    ``check_same_thread=False`` on the store's connection (plus the store's
    internal lock protecting it), the very first real chat turn raised
    ``sqlite3.ProgrammingError: SQLite objects created in a thread can only
    be used in that same thread``. This test drives ``run_agent`` from inside
    a real ``threading.Thread`` -- mirroring exactly how the app's worker
    calls it -- against the ``store`` fixture's file-backed ``MemoryStore``
    (created here, on the main/pytest thread), with a stub client so there is
    no network involved.
    """
            ...
            _Resp(
                ...
                [
                    _Block(
                        ...
                        id="t1",
                        name="learn",
                        input={"value": "loves dogs", "kind": "pref"},
                    )
                ],
            ),
            _Resp("end_turn", [_Block(type="text", text="done")]),
    ...
    def _worker() -> None:
        ...
        except BaseException as exc:  # noqa: BLE001 - hand any failure back to the main thread
            errors.append(exc)
    ...
    assert not thread.is_alive(), "run_agent did not finish inside the worker thread"
    assert errors == [], f"run_agent raised inside the worker thread: {errors!r}"
    assert outcome["result"].text == "done"

    # The store must be genuinely usable across the thread boundary: a fact
    # written by the `learn` tool call *inside* the worker thread must be
    # readable back *from the main thread* through the same connection.
    facts = store.list_facts()
    assert any(f.value == "loves dogs" for f in facts)
