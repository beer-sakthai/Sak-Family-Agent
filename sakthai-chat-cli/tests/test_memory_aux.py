...
def test_provider_initialize_with_preexisting_store(sakthai_home: Path) -> None:
    # When initialized with an existing store, initialize() must not create a new one.
    store = MemoryStore()
    store.add_fact("test fact")
    p = SakThaiMemoryProvider(store=store)
    p.initialize()  # should be a no-op, returning early on line 35
    block = p.system_prompt_block()
    assert "test fact" in block
    # Verify it's using the same store instance
    p.shutdown()
    assert p.system_prompt_block() == ""
