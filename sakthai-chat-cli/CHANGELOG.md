...
### Changed
- `scripts/setup_local_model.sh` now defaults to the **1.5B** merged model
  (`Nanthasit/sakthai-context-1.5b-merged`) and fetches the prebuilt quantized
  GGUF that ships inside the merged repos (`gguf/sakthai-<size>-Q4_K_M.gguf`),
  making local setup a single ~1 GB download with no torch/llama.cpp. The old
  convert-and-quantize pipeline remains as a fallback for repos without a
  prebuilt GGUF. Pass `0.5b` for low-RAM machines.
- `sakthai chat` visual redesign: full-width double-boxed terminal banner,
  persona-colored message panels for user and reply, live streaming inside the
  reply panel with a final Markdown render (syntax-highlighted code), styled
  persona-colored `❯` prompt with persistent history, and richer tool/error
  markers. Non-TTY output remains plain and ANSI-free.
