...
## 2026-07-29: hf-smolagents — Hugging Face Smolagents v1.26.0: CodeAgent, MCP Tools, Hub Patterns (Topic #500)

### Summary
Explored **smolagents** (28.5K ★) — HF's barebones agent library for agents that think in code. Current stable release: v1.26.0 (May 2026), dev main is v1.27.0.dev0. Key findings:

**Architecture:**
- `CodeAgent(MultiStepAgent)` — agents write actions in Python code blocks, executed via pluggable executors (local, Blaxel, E2B, Modal, Docker)
- Prompt templates stored as Jinja2 YAML files: `code_agent.yaml` (17KB), `structured_code_agent.yaml` (15KB), `toolcalling_agent.yaml` (10KB)
- Thought → Code → Observation cycles with `final_answer()` tool as terminal action
- `InferenceClientModel` wraps HF Inference API, with LiteLLM integration for non-HF providers

**MCP Integration:**
- `ToolCollection.from_mcp()` supports Stdio server, Streamable HTTP, and legacy HTTP+SSE transports
- Uses `mcpadapt` library (≥0.1.13) with `SmolAgentsAdapter` for structured output support
- `pip install smolagents[mcp]` for MCP dependencies
- `trust_remote_code=True` required for MCP tool loading
- Separate async event loop thread for MCP server communication

**Key Recent Fixes (unmerged):**
- PR #2554: Parallel tool execution fix — when one parallel tool fails, successful outputs from other tools were discarded. Fix catches per-future exceptions and writes partial successes before re-raising.
- PR #2533 (merged): Fix `{{ remaining_steps }}` Jinja2 syntax in planning YAML prompts

**Hub Integration:**
- `agent.push_to_hub()` / `agent.from_hub()` for sharing agents via HF Spaces
- `Tool.from_hub()` / `ToolCollection.from_hub()` for loading tools from Hub collections
- `Tool.from_space()` — use any HF Space as a tool

**Actionable for Sak Family:**
- Smolagents' `from_mcp()` pattern mirrors Hermes' MCP integration — both use `mcpadapt` under the hood
- The parallel tool call failure bug (PR #2554) could affect our multi-tool agent sessions — be aware that partial failures lose context
- YAML-based Jinja2 prompt templates are a clean pattern for managing agent prompts
- Sandbox executors (E2B, Modal) are cost-prohibitive for our zero-cost setup — local executor is the right choice

**Sources:**
- GitHub: https://github.com/huggingface/smolagents (main, 28.5K stars)
- PyPI: smolagents v1.26.0 (latest stable), v1.27.0.dev0 (dev)
- PR #2554: Parallel tool results fix (closed, unmerged)
- PR #2533: `remaining_steps` Jinja2 fix (merged)
- Code analysis: `agents.py` (1814 lines), `tools.py` (ToolCollection.from_mcp), `pyproject.toml` (deps)
...
## 2026-07-29: hf-transformers-tool-use-applied — Applied transformers v5.14 Tool-Use API to SakThai 7B Model Card

### Summary
Applied the transformers v5.14+ built-in tool-use chat template (`apply_chat_template(tools=...)` + `parse_response()`) improvement to the SakThai model card. This was identified as the most impactful actionable improvement from the transformers repo for SakThai's ecosystem.

**Improvement applied:**
- Updated `sakthai-context-7b-tools-README.md` (local model card for `Nanthasit/sakthai-context-7b-tools`)
- Added **Method 1: Transformers v5.14+ Tool-Use API (Recommended)** as the primary usage pattern
- Shows `apply_chat_template(tools=...)` for auto-formatting tool schemas
- Shows `parse_response()` for extracting structured tool calls
- Shows the full 5-step tool-calling lifecycle (generate → parse → append → execute → generate)
- Renamed old PEFT-only pattern to **Method 2: PEFT Adapter Loading (Legacy)**
- Updated Tool Format section to highlight v5.14+ compatibility

**Why this improvement:**
- Transformers v5.14+ provides a first-class tool-use API that eliminates manual JSON schema construction
- `get_json_schema()` auto-parses Python function signatures into JSON schemas
- `apply_chat_template(tools=...)` renders tools in the model's native chat template format
- `parse_response()` extracts structured tool calls from model output
- This makes SakThai models easier to use for developers and demonstrates they support the latest transformers features

**Related: Quantized KV Cache (`transformers.cache_utils.QuantizedCache`)**
Another key transformers v5.14+ feature applicable to SakThai models — reduces memory during inference by quantizing the KV cache (FP8, INT8). Configured via `cache_implementation="quantized"` in GenerationConfig. Most impactful for 7B models where KV cache dominates memory.

**Sources:**
- transformers v5.14+ tool-use API: https://huggingface.co/docs/transformers/en/chat_extras
- Response parsing: https://huggingface.co/docs/transformers/en/chat_response_parsing
- Quantized KV Cache: `transformers.cache_utils.QuantizedCache` (v5.14+)
- Chat templates: https://huggingface.co/docs/transformers/en/chat_templating

---

## 2026-07-29: hf-transformers-docstring-fix — Fixed missing metric_key_prefix docs in Trainer.predict()

### Summary
Identified and patched a documentation gap in `Trainer.predict()` — the `metric_key_prefix` parameter exists in the method signature but was missing from the docstring. The sibling method `Trainer.evaluate()` correctly documents this parameter. Created a complete patch with rationale, verification, and testing notes.

### Improvement Applied
- **File:** `src/transformers/trainer.py`
- **What:** Added `metric_key_prefix` parameter documentation to `predict()` docstring
- **Why:** Documentation inconsistency — users find the param in signature but no explanation
- **Cost:** Zero — docs-only change, no code modification

### Deliverables
- `transformers_predict_metric_key_prefix_doc.patch` — Ready-to-apply patch
- `transformers_improvement_result.json` — Structured analysis data
- `TRANSFORMERS_IMPROVEMENT.md` — Human-readable report
- `explore_transformers.py` — Reusable exploration script
- `run_transformers_improvement.sh` — Shell runner for cron pipeline

### Sources
- https://github.com/huggingface/transformers/blob/main/src/transformers/trainer.py
- https://github.com/huggingface/transformers/issues
- Exploratory analysis of predict() vs evaluate() docstrings

---

## 2026-07-29: huggingface_hub — Comprehensive Enhancement of SakThai HF Integration (Topic #501)

### Summary
Explored **huggingface/huggingface_hub** — the official Python library for interacting with the Hugging Face Hub (models, datasets, Spaces). Applied a concrete improvement by comprehensively enhancing the Sak-Family-Agent's huggingface_hub wrapper (`hf.py`) and CLI, plus creating a standalone ecosystem management tool.

### Improvement Applied

**1. Enhanced `hf.py` (core wrapper)** — Expanded from 2 functions to 14:
- `hf_info()` — Now supports model/dataset/space via `repo_type=` parameter
- `hf_list_models()` / `hf_list_datasets()` / `hf_list_spaces()` — Discovery APIs
- `hf_upload_file()` / `hf_upload_folder()` — File operations
- `hf_create_repo()` / `hf_delete_repo()` / `hf_repo_exists()` — Repo lifecycle
- `hf_collection_info()` / `hf_add_collection_item()` — Collection management
- `hf_list_files()` — File listing
- `hf_whoami()` — Auth verification
- `hf_ecosystem_report()` — Full ecosystem health summary
- `hf_download()` — Now supports `repo_type=` for datasets/spaces

**2. Enhanced CLI (`cli/hf.py`)** — From 2 commands to 14:
- `hf info` / `hf whoami` / `hf download` / `hf upload-file` / `hf upload-folder`
- `hf list-models` / `hf list-datasets` / `hf list-spaces` / `hf list-files`
- `hf create-repo` / `hf delete-repo` / `hf repo-exists`
- `hf collection` / `hf collection-add`
- `hf report` — One-command ecosystem health report

**3. Created standalone ecosystem manager** (`ecosystem_manager.py`):
- `report` — Full health report with download stats, collection verification, promotion suggestions
- `verify-links` — Check all model cards have cross-links to sibling models
- `check-cards` — Validate YAML front matter completeness
- `promote` — Identify low-download models needing promotion

**4. Updated test suite** (`tests/test_hf.py`):
- Added comprehensive tests for all new functions (30+ tests, up from 8)
- Shared `_fake_api_module()` fixture for consistent mocking
- CLI integration tests for all new commands

### Files Touched
| File | Action | Lines |
|------|--------|-------|
| `personas/sakthai/sakthai/hf.py` | Rewritten (2→14 functions) | 45→297 |
| `personas/sakthai/sakthai/cli/hf.py` | Rewritten (2→14 commands) | 26→218 |
| `tests/test_hf.py` | Expanded (8→30+ tests) | 145→452 |
| `ecosystem_manager.py` | New file | 307 lines |

### Key Takeaway
The previous `hf.py` was extremely minimal (only `model_info` and `snapshot_download`). The enhanced version now covers the full huggingface_hub API surface that the SakThai team needs daily: model/dataset/space listing, file uploads, repo lifecycle, and collection management. The `hf report` command gives Beer one-command visibility into the entire ecosystem's health.

### Sources
- huggingface_hub API reference: https://huggingface.co/docs/huggingface_hub/package_reference/hf_api
- Sak-Family-Agent repo: https://github.com/beer-sakthai/Sak-Family-Agent
