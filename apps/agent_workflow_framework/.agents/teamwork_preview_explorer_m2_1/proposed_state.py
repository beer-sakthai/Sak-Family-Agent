...
class StateContext:
    ...
    def interpolate(self, template: Any) -> Any:
        ...
        if isinstance(template, str):
            return self._interpolate_string(template)
        elif isinstance(template, dict):
            return {
                self.interpolate(k) if isinstance(k, str) else k: self.interpolate(v)
                for k, v in template.items()
            }
        elif isinstance(template, list):
            return [self.interpolate(item) for item in template]
        elif isinstance(template, tuple):
            return tuple(self.interpolate(item) for item in template)
        else:
            return template
