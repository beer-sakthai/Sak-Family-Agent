...
The one exception is the **System** section, which reads no runtime source at
all: it renders `src/data/system-snapshot.json`, a snapshot of this repository
(personas, architecture, tools, CLI, CI workflows, test trees) generated by
`scripts/gen_system_snapshot.py`. It is real data on every deploy, so a visit
to the bare URL of a sample-data deploy opens on System. Regenerate it with
`make system-snapshot` from the repo root; the view prints the commit it was
taken from, so a stale snapshot is visible rather than silently wrong.
