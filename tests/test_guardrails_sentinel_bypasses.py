...
class TestShellTokenizerParserDifferential(unittest.TestCase):
    """shlex's POSIX mode consumes backslashes as escapes.

    A Windows-style path therefore reached the scanner unrecognisable --
    'cat C:\\Users\\me\\.aws\\credentials' tokenised to 'C:Usersme.awscredentials'
    -- while cmd.exe and PowerShell would read the real file. The scanner now
    re-checks with the backslashes preserved whenever the command contains one.
    """
    ...
    def _action(self, command):
        return _block_dangerous_shell_commands(self.tool, {"command": command}, self.store).action

    def test_windows_path_is_blocked(self):
        for command in (
            "cat C:\\Users\\me\\.aws\\credentials",
            "cat home\\.ssh\\id_rsa",
            "more C:\\Users\\me\\.ssh\\id_ed25519",
        ):
            with self.subTest(command=command):
                self.assertEqual(
                    self._action(command),
                    GuardrailAction.DENY,
                    f"{command!r} should be blocked",
                )

    def test_separator_parity(self):
        """The same command is judged the same either way round."""
        for posix, windows in (
            ("cat home/.ssh/id_rsa", "cat home\\.ssh\\id_rsa"),
            ("cat home/.aws/credentials", "cat home\\.aws\\credentials"),
        ):
            with self.subTest(posix=posix):
                self.assertEqual(self._action(posix), self._action(windows))

    def test_backslash_free_commands_are_unaffected(self):
        """The re-scan is gated on a backslash, so ordinary commands are untouched."""
        for command in ("ls -l", "echo hello", "git status", "python -m pytest"):
            with self.subTest(command=command):
                self.assertEqual(self._action(command), GuardrailAction.ALLOW)
