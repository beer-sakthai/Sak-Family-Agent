...
#
# Every credential this server reads is remote-controlled, and
# ``secrets.compare_digest`` raises TypeError on a ``str`` carrying a non-ASCII
# character. Each of the three auth paths used to let that escape the handler
# as an unhandled exception, tearing the connection down instead of answering a
# clean 401 -- reachable unauthenticated, with no token needed.
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ("label", "path", "headers"),
    [
        ("query param", "/api/stages?token=%C3%A9", {}),
        ("query param, lone high byte", "/api/stages?bearer_token=%80", {}),
        # Request headers decode as latin-1, so a high byte arrives as non-ASCII str.
        ("bearer header", "/api/stages", {"Authorization": "Bearer \xe9"}),
        ("cookie", "/api/stages", {"Cookie": "token=%C3%A9"}),
    ],
)
def test_non_ascii_credentials_are_rejected_not_crashed(
    ...
    """A non-ASCII token is answered, not raised on."""
    base_url, _ = authed_server_base
    status, _body = _get_with_headers(f"{base_url}{path}", headers)
    assert status in (401, 403), f"{label}: expected an auth rejection, got {status}"


def test_server_still_serves_after_non_ascii_credential(
    authed_server_base: tuple[str, str],
) -> None:
    """The handler survives the bad request and still authenticates the next one."""
    import sakthai.web.server as srv

    base_url, _ = authed_server_base
    # Pin the expected token here rather than trusting the module-scoped
    # fixture's: the CLI regen-token tests mutate this same global, so which
    # token is live depends on test order.
    token = "non_ascii_regression_token"
    srv._BEARER_TOKEN = token

    _get_with_headers(f"{base_url}/api/stages?token=%C3%A9", {})
    status, body = _get_with_headers(f"{base_url}/api/stages", {"Authorization": f"Bearer {token}"})
    assert status == 200
    assert "kpis" in body


def test_token_matches_is_total_over_non_ascii() -> None:
    """The comparison helper returns False rather than raising."""
    from sakthai.web.server import _token_matches

    assert _token_matches("abc", "abc") is True
    assert _token_matches("abc", "abd") is False
    assert _token_matches("\xe9", "abc") is False
    assert _token_matches("abc", "\xe9") is False
