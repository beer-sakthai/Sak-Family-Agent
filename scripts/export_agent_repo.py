...
# Files to drop from a copied tree, keyed by the tree's relative path in
# COPY_DIRS and named relative to that tree.
#
# `.github/dependabot.yml` configures ~20 directories by their monorepo paths
# (`/personas/sakthai/sakthai/telegram`, `/services/teams-copilot-mcp`, ...).
# This exporter *flattens* those paths -- `personas/sakthai/sakthai` becomes
# `sakthai`, `personas/sakthai/agent-self-evolution` becomes
# `packages/agent-self-evolution` -- so every directory in that config resolves
# to nothing in an exported repo. Copying it verbatim turns one correct config
# into six broken ones, each silently updating nothing. Exported repos get the
# minimal config rendered by `_render_dependabot()` instead.
EXCLUDED_TREE_FILES = {
    ".github": frozenset({"dependabot.yml"}),
}
...
def _copy_tree(src: Path, dst: Path, exclude_files: frozenset[str] = frozenset()) -> None:
    if not src.exists():
        return
...
# Manifest names that make a directory worth a Dependabot entry in an exported
# repo. `*-requirements.in` files paired with a hash-pinned `.lock` are excluded
# deliberately: Dependabot regenerates a `.txt` beside an `.in`, never a
# `.lock`, so an entry there bumps the input and leaves the file that is
# actually installed from untouched.
_DEPENDABOT_UV = ("pyproject.toml", "uv.lock")
_DEPENDABOT_PIP_GLOBS = ("*requirements*.txt", "*requirements*.in", "pyproject.toml")
_DEPENDABOT_SKIP_DIRS = {"node_modules", ".venv", "venv", "build", "dist", "__pycache__"}


def _dependabot_directories(out: Path) -> dict[str, list[str]]:
    """Find the manifests an exported repo actually contains, by ecosystem.

    The monorepo config cannot be reused and a fixed list cannot either: the
    export *flattens* paths (`personas/sakthai/sakthai` -> `sakthai`,
    `personas/sakthai/agent-self-evolution` -> `packages/agent-self-evolution`)
    and each persona carries a different skill overlay. A SakThai export has 11
    manifest directories; a SakTan export has far fewer. So scan the tree that
    was actually produced.
    """
    found: dict[str, set[str]] = {"uv": set(), "pip": set(), "npm": set()}
    # `rglob` yields only descendants, so the repository root -- which holds the
    # uv project and is the single most important entry -- has to be added back
    # explicitly. Leaving it out produced a config with every persona manifest
    # and no root.
    for path in [out, *out.rglob("*")]:
        if not path.is_dir() or path.is_symlink():
            continue
        rel_parts = path.relative_to(out).parts
        if any(part in _DEPENDABOT_SKIP_DIRS for part in rel_parts):
            continue
        rel = "/" + path.relative_to(out).as_posix() if rel_parts else "/"

        if all((path / name).is_file() for name in _DEPENDABOT_UV):
            found["uv"].add(rel)
            continue
        if (path / "package.json").is_file():
            found["npm"].add(rel)
        # A directory whose only python manifest is an `.in` with a `.lock`
        # sibling is skipped -- see the note on _DEPENDABOT_PIP_GLOBS.
        pip_hits = [m for glob in _DEPENDABOT_PIP_GLOBS for m in path.glob(glob)]
        useful = [m for m in pip_hits if not (m.suffix == ".in" and m.with_suffix(".lock").is_file())]
        if useful:
            found["pip"].add(rel)

    # The root is always the uv project, even though `packages/` copies below it
    # look like standalone ones.
    return {eco: sorted(dirs) for eco, dirs in found.items() if dirs}


def _render_dependabot(out: Path) -> str:
    """The dependabot config for an *exported* repo, not the monorepo one.

    Generated from the exported tree rather than copied, because the monorepo
    config names directories by their monorepo paths and the export flattens
    them -- every entry would resolve to nothing. See
    tests/test_export_agent_repo.py.
    """
    by_eco = _dependabot_directories(out)

    lines = [
        "# Generated by scripts/export_agent_repo.py -- do not edit by hand.",
        "#",
        "# This is NOT the monorepo config. Sak-Family-Agent/.github/dependabot.yml",
        "# names directories by their monorepo paths; the export flattens those",
        "# (personas/sakthai/sakthai -> sakthai), so none of them would resolve here.",
        "# These entries were derived from the files this export actually contains.",
        "version: 2",
        "",
        "updates:",
    ]

    for ecosystem in ("uv", "pip", "npm"):
        directories = by_eco.get(ecosystem)
        if not directories:
            continue
...
def export_agent_repo(persona: str, out: Path) -> Path:
    ...
    for rel in COPY_DIRS:
        src = REPO_ROOT / rel
        _copy_tree(src, out / rel, EXCLUDED_TREE_FILES.get(rel, frozenset()))
    ...
    (out / ".github" / "dependabot.yml").write_text(_render_dependabot(out), encoding="utf-8")
