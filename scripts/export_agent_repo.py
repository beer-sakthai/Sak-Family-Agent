...
def _render_dependabot() -> str:
    """The dependabot config for an *exported* repo, not the monorepo one.

    The monorepo config names ~20 directories by their monorepo paths, and this
    exporter flattens those paths, so copying it verbatim would produce a config
    in which every directory resolves to nothing. An exported repo has only two
    manifests worth watching, both at the root, so that is all this declares.
    """
    return """# Generated by scripts/export_agent_repo.py -- do not edit by hand.
#
# This is NOT the monorepo config. Sak-Family-Agent/.github/dependabot.yml names
# ~20 directories by their monorepo paths; the export flattens those paths
# (personas/sakthai/sakthai -> sakthai), so none of them would resolve here. An
# exported repo carries one uv project at the root plus its workflows, so that
...
def export_agent_repo(persona: str, out: Path) -> Path:
    ...
    (out / ".github" / "dependabot.yml").write_text(_render_dependabot(), encoding="utf-8")
