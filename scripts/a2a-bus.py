...
def save_msgs(msgs):
    with lock, open(MSG_FILE, "w") as f:
...
def save_tasks(tasks):
    with lock, open(TASKS_FILE, "w") as f:
...
class A2AHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if self.path == "/send":
            ...
            msgs = load_msgs()
            targets = {agent, "all"}
            inbox = [m for m in msgs if m["to"] in targets]
            ...
            save_tasks(tasks)
            self.send_json(
                {"status": "ok", "task_id": task["task_id"], "shards": len(task["shards"])}
            )
            return
            ...
            if (
                shard is None
                and isinstance(shard_id, int)
                and 0 <= shard_id < len(task.get("shards", []))
            ):
                # Try numeric index
            ...
            if shard is None:
                self.send_json(
                    {"status": "error", "message": f"shard {shard_id} not found in task {task_id}"}
                )
                return
            ...
            if shard.get("status") == "claimed":
                self.send_json(
                    {"status": "error", "message": "shard already claimed by another agent"}
                )
                return
            ...
            save_tasks(tasks)
            self.send_json(
                {"status": "ok", "task_id": task_id, "shard_id": shard_id, "input": shard}
            )
            ...
            all_done = all(
                ...
                for s in task.get("shards", [])
            )
            task["status"] = "completed" if all_done else "in_progress"
            task["updated"] = time.time()
            save_tasks(tasks)
            self.send_json(
                {"status": "ok", "task_id": task_id, "shard_id": shard_id, "all_done": all_done}
            )
