...
Inside `.github/workflows/`, everything Scorecard's Pinned-Dependencies check
re-scans is pinned by digest: every `uses:` by commit SHA, both Dockerfiles'
base images by image digest, and the pip installs by hash rather than by
version — `bandit` from `.github/bandit-requirements.lock`, `pylint` from
`.github/pylint-requirements.lock`, `zizmor` from
`.github/zizmor-requirements.lock`, and `apps.yml`'s two Python jobs from
`.github/apps-requirements.lock`, all generated by `scripts/gen_hash_lock.py`
(never hand-edited; the command is in each lock's header). A bare
`pip install pkg==x.y.z` is still a finding.

**`Quality gates` is a required status check, and it duplicates `ci.yml`.**
That job in `repository-security-quality.yml` runs the same
ruff/ruff-format/mypy/pytest chain at the same `--cov-fail-under=96` that
`ci.yml` already runs across **3.11 and 3.12**, so it is a strict subset and
every PR pays for the suite twice. Deleting it does **not** relax the
requirement — it strands it, and GitHub refuses the merge with
`Required status check "Quality gates" is expected.` (HTTP 405). This was tried
and reverted in #1402. Removing the duplication needs a repository *settings*
change first: drop `Quality gates` from the required-checks list for `main`
(Settings → Branches), which no commit can do. Do that, then delete the job —
`ci.yml` covers everything it did, on both Python versions.

**zizmor audits every workflow, offline.** `repository-security-quality.yml`
runs `zizmor --no-online-audits --config .github/zizmor.yml .github/workflows/`.
Two things are load-bearing and easy to undo by accident. It scans the
*directory*: it previously named one file — itself — so 25 of 26 workflows were
never audited. And `--no-online-audits` is deliberate: zizmor's online audits
call the GitHub API and abort the entire run on any failure there, so a
transient API blip would read as a security regression. It runs at the default
persona, not `--pedantic`; pedantic adds ~50 findings that are almost entirely
style (`undocumented-permissions`, `template-injection` on `${{ matrix.* }}`).
`.github/zizmor.yml` carries exactly two suppressions, each with its reason —
`labeler.yml`'s `pull_request_target` (safe: no checkout of PR head) and
`sonarcloud.yml`'s archived action (a real finding, pending migration). Dependabot's
`uv`/`npm`/`github-actions`/`docker` ecosystems are what keep those pins current.

**Do not read that as "Scorecard is clean".** Re-derive the live picture rather
than trusting this paragraph — the run on `e9c9bfc` reported **46** findings,
not the two this file used to claim. Pull the SARIF from the latest
`ossar.yml` run on `main` (the GitHub Security tab needs a `security_events`
token that agent sessions do not get, but the workflow's `SARIF file` artifact
is readable through the Actions API):

```bash
curl -sS -H "Authorization: Bearer $GH_TOKEN" \
  "https://api.github.com/repos/beer-sakthai/Sak-Family-Agent/actions/artifacts?per_page=20"
# then download the "SARIF file" artifact's zip and read runs[].results[]
```

As of 2026-09-15 that leaves **~34 open Pinned-Dependencies findings outside
`.github/`** — `pipCommand`/`npmCommand`/`downloadThenRun` in `Dockerfile`s
under `infra/`, persona skill setup scripts (`SakKing-comfyui`,
`Sak*-github-auth`, `agent-self-evolution/evolve_agent.sh`), `scripts/bootstrap.sh`,
`scripts/setup-extensions.sh`, and the `sakthai-chat-cli/` copies of several of
those. They are untouched on purpose: those scripts are developer-run setup, not
CI, and `sakthai-chat-cli/` is a folded-in tree that is deliberately not synced.
Two `pip install -e ".[dev]"` findings (`pylint.yml`, `agent-self-evolution.yml`)
also remain — the editable local install is pinned by construction, but the dev
extra it resolves is not.

Branch-Protection, Code-Review, CII-Best-Practices and Fuzzing are repository
*settings* or programme choices, not repository content — no commit closes them.
