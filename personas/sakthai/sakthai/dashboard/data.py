...
def _parse_team_roster(soul_path: Path | None = None) -> list[dict[str, str]]:
    ...
    try:
        ...
        for line in lines:
            ...
            if "|Agent|Handle|Role|Model|" in line.replace(" ", ""):
                ...
                continue
            if in_table:
                # Separator rows are made only of pipes, dashes, colons, spaces.
...
def collect_dashboard_data(days: int = 30, store: MemoryStore | None = None) -> dict[str, Any]:
    ...
    ``store`` may be injected (e.g. by tests); when omitted a store is opened
    on the default database path for the duration of the call.
    """
    try:
        if store is not None:
            return _collect(store, days)
        with MemoryStore() as own_store:
            return _collect(own_store, days)
    except Exception as e:
...
def _collect(store: MemoryStore, days: int) -> dict[str, Any]:
    ...
    for rf in revenue_facts:
        ...
        date_str = payload.get("date") or datetime.fromtimestamp(rf.created_at, UTC).strftime(
            "%Y-%m-%d"
        )
