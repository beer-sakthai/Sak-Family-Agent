...
## Scripting llama-cli output (pitfalls, verified 2026-07-31)

When capturing `llama-cli -f prompt.txt` output in a script/subprocess, do NOT assume the stdout is just the model generation:

1. **Prompt echo is truncated, not exact.** llama-cli echoes the prompt to stdout prefixed with `> ` and truncates long prompts with `...(truncated)`. Exact-string removal of the prompt FAILS on long prompts → leftover prompt text (with tool names like `<tool_call>`) pollutes the "generation" and inflates quality metrics. Correct extraction: cut at the perf footer, then take everything AFTER the last `(truncated)` marker.
2. **Perf footer format:** modern builds print `[ Prompt: 68.6 t/s | Generation: 24.8 t/s ]` (NOT `llama_perf_context_print` / `eval time` — those are from older builds). Parse with `re.search(r"Generation:\s*([\d.]+)\s*t/s", raw)`.
3. **Flag spelling:** this build accepts `-no-cnv` (single dash) / `--no-conversation`; `--no-cnv` (double dash) is an INVALID argument → exit 1 with 0 output. Use `-no-cnv -st` for one-shot non-interactive.
4. **Token counts:** llama-cli does not print token counts in this footer. Count them yourself with the matching HF tokenizer (`AutoTokenizer.from_pretrained(base_model_id)`).
5. **Verification-first:** a status 200 with empty output means the flags were wrong — check `proc.returncode != 0` AND `len(gen_text) == 0` before recording success. Delete bad YAML reports with `HfApi().delete_file()` (the raw `DELETE /api/models/{repo}/unpack/...` endpoint returns 404; use the SDK).
