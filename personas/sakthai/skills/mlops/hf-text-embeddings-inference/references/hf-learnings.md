...
## Deepening: Latest Model Support & Features (2026-07-24)

### Updated: Supported Model Architectures

Since the initial coverage, TEI has added support for several new architecture families:

| Architecture | Examples | Position Encoding | Added In |
|---|---|---|---|
| **BERT** | `WhereIsAI/UAE-Large-V1` | Absolute | Original |
| **NomicBERT** | `nomic-ai/nomic-embed-text-v1.5` | Absolute | Original |
| **JinaBERT** | `jinaai/jina-embeddings-v2-base-en` | Alibi | Original |
| **XLM-RoBERTa** | `intfloat/multilingual-e5-large-instruct` | Absolute | Original |
| **Mistral** | `Salesforce/SFR-Embedding-2_R` | Rope | v1.5+ |
| **Alibaba GTE** | `Alibaba-NLP/gte-Qwen2-7B-instruct` | Rope | v1.5+ |
| **Qwen2 Embed** | `Alibaba-NLP/gte-Qwen2-1.5B-instruct` | Rope | v1.5+ |
| **MPNet** | `sentence-transformers/all-mpnet-base-v2` | Absolute | v1.5+ |
| **ModernBERT** | `answerdotai/ModernBERT-large` | Absolute | v1.8+ |
| **Qwen3 Embed** | `Qwen/Qwen3-Embedding-0.6B` | Rope | v1.9+ |
| **Gemma3** | `google/embeddinggemma-300m` (gated) | Rope | v1.9+ |
| **CamemBERT** | Sequence Classification | Absolute | v0.4.0 |
| **RoBERTa** | `SamLowe/roberta-base-go_emotions` | Absolute | v0.4.0 |

### MTEB Top-Ranked Supported Models

| MTEB Rank | Model Size | Architecture | Model ID |
|---|---|---|---|
| 2 | 7.57B | Qwen3 | `Qwen/Qwen3-Embedding-8B` |
| 3 | 4.02B | Qwen3 | `Qwen/Qwen3-Embedding-4B` |
| 4 | 509M | Qwen3 | `Qwen/Qwen3-Embedding-0.6B` |
| 6 | 7.61B | Qwen2/GTE | `Alibaba-NLP/gte-Qwen2-7B-instruct` |
| 7 | 560M | XLM-RoBERTa | `intfloat/multilingual-e5-large-instruct` |
| 8 | 308M | Gemma3 | `google/embeddinggemma-300m` (gated) |
| 15 | 1.78B | Qwen2/GTE | `Alibaba-NLP/gte-Qwen2-1.5B-instruct` |
| 18 | 7.11B | Mistral | `Salesforce/SFR-Embedding-2_R` |
| 35 | 568M | XLM-RoBERTa | `Snowflake/snowflake-arctic-embed-l-v2.0` |
| 41 | 305M | GTE | `Snowflake/snowflake-arctic-embed-m-v2.0` |
...
| N/A | 475M | NomicBERT | `nomic-ai/nomic-embed-text-v2-moe` |
| N/A | 434M | GTE | `Alibaba-NLP/gte-large-en-v1.5` |
...
Models marked "Very Expensive" (>2B params) benefit from the `--max-batch-tokens` tuning (see below).

### Sequence Classification & Reranker Support

TEI now supports more than just embeddings — it serves **Sequence Classification** models for tasks like reranking and sentiment analysis.
...
#### Sequence Classification Endpoint
...
The `/predict` endpoint returns classification labels and scores — works with any supported Sequence Classification model.

### ONNX Weight Loading

TEI now supports ONNX weight loading in addition to Safetensors. This is enabled via the `ort` feature flag:

```bash
# Install with ONNX backend (x86)
cargo install --path router -F ort
```

ONNX support enables:
- Broader hardware compatibility (CPU-optimized models)
- Integration with ONNX Runtime optimization passes
- Combined Safetensors + ONNX fallback loading

### Apple Silicon & Metal Support

TEI provides a prebuilt binary via Homebrew for Apple Silicon (M1/M2/M3/M4):

```bash
brew install text-embeddings-inference
```

Launch with Metal acceleration:
...
**Zero-cost implication:** Apple Silicon users can run TEI locally without any cloud GPU costs. Models up to ~500M params perform well on M-series hardware.

### Token-Based Dynamic Batching — Deep Dive

TEI uses **token-based dynamic batching** (not request-count-based). This means:

```python
# Request A: short text (16 tokens)
# Request B: long text (512 tokens)
# Request C: medium text (128 tokens)
# Instead of 3 separate forward passes,
# TEI packs them into ONE batch sized by token count:
# Batch: 16 + 512 + 128 = 656 tokens
```

**Key tunable: `--max-batch-tokens`** (default: 16384)

| Setting | Behaviour | Best For |
|---|---|---|
| Low (4096) | Small batches, low latency per request | Real-time apps, latency-sensitive |
| Medium (16384) | Balanced throughput/latency | General purpose (default) |
| High (65536) | Large batches, max throughput | Batch processing, embeddings at scale |

**Formula:** `optimal_max_batch_tokens ≈ avg_seq_len * desired_batch_size * 1.2`

Example: average sequence 256 tokens, batch size 64 → `256 * 64 * 1.2 = 19660`, round to 20480.

### New CLI Arguments (v1.9+)
...
- Homebrew: `brew install text-embeddings-inference`
- Docker tags: `ghcr.io/huggingface/text-embeddings-inference:{cuda,cpu,rocm}-{version}[-grpc]`
