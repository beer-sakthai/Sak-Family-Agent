...
author: SakThai
license: MIT
description: "Scheduled crawl of HF Hub trending models: fetch, select uncovered model, deep-dive, report, update tracker, sync to GitHub. Also hosts learn-hf-topic workflow (see references/) for systematic ecosystem coverage expansion."
version: 1.2.0
category: mlops
...
> **Sibling workflow:** `references/learn-hf-topic.md` documents the parallel cron that systematically covers HF ecosystem *topics* (libraries, tools, SDKs) instead of trending models. Same class of work — systematic coverage expansion — different target.
...
**Alternative endpoint:** If `/api/trending` is unavailable or returns unexpected results, use `https://huggingface.co/api/models?sort=trendingScore&direction=-1&limit=30` which ranks by trending score directly on the Models API.
...
|---------|---------|------------|
| `sort=trending` on `/api/models` | 400 Bad Request | Use either A) `/api/trending` (canonical) or B) `/api/models?sort=trendingScore&direction=-1&limit=30` — both return trending data via different API endpoints. `sort=trendingScore` is the valid parameter name for `/api/models` specifically. |
