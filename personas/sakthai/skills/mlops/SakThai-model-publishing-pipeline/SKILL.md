...
**CRITICAL — HF_TOKEN must be optional**: The dataset is public, so downloading doesn't need auth. The training cells should run even when HF_TOKEN isn't set. Only the push-to-Hub cell needs the token. Use a 3-tier fallback:
...
This tries (1) Kaggle Secrets, (2) environment variable, (3) local HF cache. First two may fail on Kaggle if the secret isn't set; the third handles the case where the HF token is already logged in. The push cell checks `if HF_TOKEN:` before attempting to upload.
| 4 | Load + format dataset — **Critical: handle both `messages` and `conversations` keys.** Legacy v6 data uses `conversations` in some rows. Use `safe_render()` that checks both keys and **skips malformed rows individually** via try/except so one bad row doesn't crash the entire training. Always print `(from X rows)` so the skip count is visible. Pattern: `def safe_render(rec): try: msgs = rec.get(\"messages\") or rec.get(\"conversations\") or []; ... return tok.apply_chat_template(...) except: return None`. |
...
### Pre-Upload: Check Existing Repo Contents

**Before uploading any file, verify it doesn't already exist in the target repo.** Failing to check creates duplicates that waste bandwidth, clutter the repo, and need cleanup.

```python
from huggingface_hub import HfApi
api = HfApi()

# List existing files in the target repo
siblings = api.get_repo_info(repo_id="org/model-name", repo_type="model").siblings
existing = [s.rfilename for s in siblings]
print(f"Existing files ({len(existing)}):")
for f in existing:
    print(f"  {f}")
```

If your file already exists (possibly under a different path like `gguf/name.gguf`), decide your approach:
- **Overwrite** — upload anyway if your version is newer/better; the upload creates a new commit
- **Delete old + re-upload** — use `api.delete_file(path_in_repo="old/path", repo_id="repo", repo_type="model")`
- **Keep both** — use the existing file and skip the upload

If the file exists under a subdirectory (`gguf/`) but you want it at the root (more discoverable for llama.cpp/Ollama users), delete the subdirectory version after upload.

### Push everything to HF

**⚠️ GGUF/LFS files:** Use the `hf upload` CLI for GGUF files. The Python `upload_file()` API silently creates empty 0 MB LFS stubs. See `references/gguf-upload-verification.md`.

```bash
# Upload GGUF via CLI (works for LFS)
hf upload org/model-name ./merged/model-Q4_K_M.gguf model-Q4_K_M.gguf --repo-type model
```

```python
# List existing files (always check first)
siblings = api.get_repo_info(repo_id="org/model-name").siblings
existing = [s.rfilename for s in siblings]

# Upload non-LFS files (JSON, configs, reports) via Python API — this is fine
...
| `references/model-card-enrichment-workflow.md` | Pre-flight narrative audit, broken-code detection, branding, dynamic badges, pipeline integration, family cross-link tables, post-upload verification checklist |
...
| `references/dataset-safety-cycle-enrichment.md` | Safety guardrails + cycle workflow data patterns. How to audit tool balance, generate refusal examples, and add 6-stage workflows. Added 2026-07-29 after v7 safety gap. |
...
| `references/gguf-upload-verification.md` | GGUF LFS upload pitfall (`hf upload` CLI ✅ vs `upload_file()` API ❌), verification via range downloads, HF Spaces free tier limits, recovery steps. Added 2026-07-28. |
...
| `references/training-data-visibility-kaggle.md` | Private HF datasets need HF_TOKEN or public visibility for Kaggle training. Fixes and fallback patterns. Added 2026-07-29. |
...
| `references/eval-results-workflow.md` | Adding `.eval_results/*.yaml` YAML files to model repos for Community Evals — format, upload, batch workflow. Added 2026-07-29 after adding to all 12 SakThai models. |
...
- **Kaggle kernel papermill error — No kernel name found**: When pushing a notebook via CLI, papermill fails with `ValueError: No kernel name found in notebook and no override provided`. Root cause: missing `kernelspec` + `language_info` in notebook metadata. Three approaches:
  - **Approach 1 (preferred — proper metadata)**: Create notebook with ALL required fields. Tested 2026-07-29: this metadata structure pushes via CLI without papermill errors:
    ```json
    {
      "metadata": {
    ...
    ```
    **Key finding:** The existing pitfall said 'even with kernelspec.name: python3 the issue occurs' — the missing piece was `language_info.name: python`. Papermill validates both; missing either triggers the error.
  - **Workaround A (pull native format)**: `kaggle kernels pull <owner/slug> -p <dir> -m`, replace only cell content, push.
  - **Workaround B (manual URL import)**: Import via URL on kaggle.com — web UI assigns Python 3 kernel, bypassing papermill.

- **CUDA kernel mismatch on Kaggle T4 with unsloth**: After installing unsloth via `pip install unsloth`, the `FastLanguageModel.from_pretrained()` call crashes with `AcceleratorError: CUDA error: no kernel image is available for execution on the device`. Root cause: `pip install unsloth` upgrades PyTorch to a version whose pre-compiled CUDA kernels don't match Kaggle's T4 GPU driver. **Fix:** Use Kaggle's pre-installed PyTorch (which matches the T4 driver) and only pip-install unsloth without its torch dependency by not upgrading torch. The safest install sequence is:
  ```python
  %%capture
  # CRITICAL: --no-build-isolation prevents pip from upgrading torch.
  # Kaggle's pre-installed torch matches their GPU (T4 sm_75 or P100 sm_60).
  # Upgrading torch breaks CUDA kernel compatibility.
  !pip install --upgrade --no-build-isolation unsloth
  !pip install --upgrade --no-deps trl peft accelerate bitsandbytes
  ```
  Do NOT install or upgrade `torch`, `torchvision`, `torchaudio`, or `cuda` packages. Kaggle's built-in torch is the correct version for their GPU. The `--no-build-isolation` flag is the critical fix — it prevents unsloth's pip install from triggering a torch rebuild that produces CUDA kernels incompatible with the T4 (sm_75) or P100 (sm_60). Without this flag, `FastLanguageModel.from_pretrained()` crashes with `AcceleratorError: CUDA error: no kernel image is available for execution on the device`. If unsloth still fails, restart the Kaggle kernel and re-run — GPU assignment varies across sessions.
  
  - **Diagnostic:** Check Kaggle's torch version: `!python -c "import torch; print(torch.__version__); print(torch.cuda.is_available())"`. If this works but unsloth crashes, the issue is unsloth's torch dependency, not the GPU.
  
  **Automation mandate:** Never ask Beer to do this manually. Automate everything.
...
- **Private HF dataset blocks Kaggle training**: If the training dataset is private on HF, Kaggle's notebook environment can't download it without a valid HF_TOKEN. The `token=None` fallback in `hf_hub_download()` fails for private repos. Two fixes:
  - **Fix A (make dataset public)**: If the dataset contains no sensitive data, update visibility via `HfApi().update_repo_settings(repo_id=..., private=False, repo_type='dataset')`. This is the simplest approach — Kaggle can read the data without any auth.
  - **Fix B (set HF_TOKEN on Kaggle)**: Add the HF_TOKEN as a Kaggle Secret (requires UI interaction) or pass via `env_vars` in kernel-metadata.json. Use the 3-tier fallback in the auth cell.
  - **Fix C (use Colab)**: Colab's userdata API is more reliable for token auth. Keep a Colab notebook variant.
  - **Diagnostic**: If the kernel errors at `hf_hub_download()` with `HTTP 401` despite the try/except, the dataset is likely private. Check visibility via `curl -s https://huggingface.co/api/datasets/OWNER/NAME | python3 -c \"import json,sys; print(json.load(sys.stdin).get('private'))\"`.
- **Dataset key mismatch when concatenating versions**: The v5 dataset used `conversations` as the conversation key, while v6+ uses `messages`. When appending v5 data to generate v6/v7, 200 rows kept the old `conversations` key. The notebook render function that reads `rec["messages"]` crashes with `KeyError`. **Fix in dataset generation:** Normalize all rows to `messages` before uploading. **Fix in notebook:** Use a safe render function that checks both keys: `msgs = rec.get("messages") or rec.get("conversations")` and skips rows where neither exists.\n- **trl/SFTConfig version incompatibility with Unsloth**: When using Unsloth's `SFTTrainer` wrapper on Kaggle or Colab, install `trl==0.12.0` explicitly. Newer trl versions (0.13+) changed `SFTConfig` causing `TypeError: SFTConfig.__init__() got an unexpected keyword argument 'push_to_hub_token'` because Unsloth's wrapper injects this parameter. Fix: replace `!pip install trl` with `!pip install --upgrade --no-deps trl==0.12.0 peft accelerate bitsandbytes`.
...
- **GGUF already on HF under `gguf/` subdirectory**: GGUFs are often stored under `gguf/` subdirectories but are more discoverable at root level. Before uploading, use `api.get_repo_info().siblings` to list existing files. If the GGUF exists under `gguf/`, delete that version and upload to root instead. Failing to check creates duplicates and wastes upload bandwidth.
- **`upload_file()` silently fails for LFS files**: The Python `HfApi.upload_file()` method creates empty 0 MB git pointer stubs instead of uploading the actual LFS blob. The upload progress bar shows 100% but the file isn't stored. Always use `hf upload REPO_ID LOCAL_PATH REMOTE_PATH --repo-type model` for GGUF/large binary files. Verify with `curl -r 0-1023` (expect HTTP 206). See `references/gguf-upload-verification.md` for full details and recovery steps.
- **HF Spaces interactive requires PRO**: Gradio, Streamlit, and Docker Spaces return HTTP 402 without a PRO subscription ($9/mo). Static HTML/JS/CSS Spaces are the only free tier option. See `references/gguf-upload-verification.md` for comparison table.
