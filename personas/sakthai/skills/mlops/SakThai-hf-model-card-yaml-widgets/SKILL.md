...
- **`config` field under metrics in model-index MUST be a string** — The HF validation API rejects nested objects for `config` in `model-index[].results[].metrics[].config`. Do NOT write:
  ```yaml
  metrics:
    - type: accuracy
      value: 0.95
      config:
        scale: 1-5    # ❌ Nested object — ValueError
  ```
  Instead, use a plain string:
  ```yaml
  metrics:
    - type: accuracy
      value: 0.95
      config: scale-1-5   # ✅ Plain string passes validation
  ```
  Error observed: `"model-index[0].results[0].metrics[1].config" must be a string`.
...
For recurring (cron-driven) maintenance — one improvement per run, focused on underperforming models (<50 dl):

1. **Inventory** — list all your models via `api.list_models(author=...)`, sort by downloads ascending
2. **Identify** — pick the lowest-download model whose card has missing YAML (no `pipeline_tag`, `license`, `base_model`) or an outdated family table
3. **Enrich** — add missing fields, expand tags, update family cross-links, add usage examples
4. **Verify** — download the card back after upload, check fields render correctly
5. **Record** — log the improvement in SOUL.md or a growth-cycle tracker

**Critical: cross-table staleness scan.** When updating download counts in a family table, search the ENTIRE card for ALL occurrences — a model card with 2+ tables (Family + Pipeline Integration + hero stats) almost always has stale numbers in every table. Fix them all in one pass. See `references/model-card-audit-workflow.md` → Cross-Table Staleness Detection for the full checklist.

Full workflow in [`references/model-card-audit-workflow.md`](references/model-card-audit-workflow.md) — including priority matrix of improvements, step-by-step code, and common pitfalls.

### Narrative Consistency Audit

When a model family grows to 10+ repos, narrative drift is inevitable — later additions get technical depth but miss the emotional anchor present on the flagship cards. Run a **narrative consistency audit** to ensure every card tells the same origin story.

Full workflow in [`references/narrative-consistency-audit.md`](references/narrative-consistency-audit.md) — marker checklist, audit script template, narrative section template, upload/verify pattern, and common pitfalls.

### Pipeline Integration Diagrams

When managing a **multimodal model family**, add an ASCII art pipeline diagram showing how models connect end-to-end. This helps visitors understand the system architecture and discover sibling models they might otherwise miss.

Place after the branded header or in a dedicated `## Pipeline Integration` section:
...
User Input (image/question)
      ...
      ▼
┌─────────────────┐
│  Vision 7B      │  image-to-text
│  (LLaVA GGUF)   │
└────────┬────────┘
         │ text description
         ▼
┌──────────────────────────────┐
│ Embedding Multilingual       │  cross-lingual retrieval
│ 384-dim, 50+ languages      │
└────────┬─────────────────────┘
         │ enriched context
         ▼
┌──────────────────────┐
│ Context 1.5B / 7B    │  reasoning, tool-calling
└────────┬─────────────┘
         │ response text
         ▼
┌──────────────────────┐
...
| Stage | Model | Role |
|-------|-------|------|
| 1. Input | Vision 7B | Understand images, diagrams, screenshots |
| 2. Retrieve | Embedding Multilingual | Cross-lingual search (50+ languages) |
| 3. Reason | Context 1.5B | Generate text, call tools |
| 4. Output | TTS Model | Synthesize speech |
````

**Guidelines:**
- Use box-drawing chars (`─ │ ┌ ┐ └ ┘ ┤ ├`) — they render in all Markdown viewers
- Show data flow direction explicitly (arrows or `▼`)
- Include a stage table after the diagram for quick scanning
- Link each stage's label to the sibling model page
- Keep <40 lines total — full pipeline fits in one screen
- Mark "You are here" on the current model's stage with an arrow label
...
### Model-Index with Expected Benchmarks (GGUF / Non-Inferable Models)

Many models on the Hub (GGUF quantized, custom architectures, or models without inference API support) **cannot run on HF serverless inference** but still benefit from structured benchmark display via `model-index`. Use `verified: false` and source-scoped descriptions to indicate these are upstream/expected values, not live measurements:
...
- name: sakthai-vision-7b
  ...
  - task:
    ...
    - type: accuracy
      value: 78.5
      name: Accuracy
      verified: false        # ← marks as expected, not measured
```

**Rules:**
- **Always** set `verified: false` when the score is from the upstream paper, not a measurement of this specific build
- Add a prose caveat in the card body: "from base model evaluation, Q4_K_M retains ≥97%"
- List separate benchmarks as independent entries (one per dataset + task combo)
- Do NOT set `verified: true` unless you've run multi-trial evaluation on the exact model files in the repo
- The `name` under each task should describe the evaluation task clearly — `Visual Question Answering`, `Scene Understanding`, etc.

**When to use this pattern:**
- GGUF quantized models where only upstream FP16 scores exist
- Models that depend on external companion files (mmproj, tokenizers) not inferable on HF
- Models too large for free-tier serverless inference
- Any model where actual multi-trial evaluation is pending
...
| [`references/model-card-audit-workflow.md`](references/model-card-audit-workflow.md) | Cron-driven incremental improvement workflow — audit owned ecosystem, identify underperforming cards, enrich YAML, update family tables, one improvement per run |
| [`references/model-index-verification.md`](references/model-index-verification.md) | How to verify model-index YAML was parsed correctly via HF API (`card_data.eval_results`) — catches silent YAML failures that grep-based checks miss |
| [`references/narrative-consistency-audit.md`](references/narrative-consistency-audit.md) | Narrative/storytelling consistency audit across model family — marker checklist, template, upload/verify pattern |
