---
name: SakThai-hf-hub-models-api
author: SakThai
license: MIT
description: "Hugging Face Hub Models REST API — search, filter, paginate, and inspect models programmatically. Covers list/search endpoints, cursor pagination, model metadata, siblings, safetensors info, gated access, inference config, and practical zero-cost patterns."
version: 1.0.0
platforms: [linux, macos]
category: mlops
tags: [huggingface, hub, api, models, rest, search, discovery]
---

# Hugging Face Hub Models API

Deep reference for the Models REST API on Hugging Face Hub. Use these endpoints
to discover, search, inspect, and filter the 1M+ models on the Hub — all
without consuming inference credits.

See also [`references/hf-cli-model-listing.md`](references/hf-cli-model-listing.md)
for the `hf` CLI approach to listing and filtering models by author.
