...
license: MIT
description: >
  Complete reference for Hugging Face Spaces YAML configuration system —
  SDK options, hardware tiers, metadata fields, environment variables,
  OAuth, preloading, custom headers, and advanced configuration for
  Gradio, Streamlit, Docker, and Static Spaces.
version: 1.0.0
metadata:
  hermes:
    tags: [huggingface, spaces, configuration, yaml, deployment, sdk, gradio, docker]
    category: mlops
category: mlops
---
...
### CPU
| Flavor | vCPU | RAM | Disk | Hourly Price | Notes |
|--------|------|-----|------|-------------|-------|
| `cpu-basic` | 2 | 16 GB | 50 GB | Free | ⚠️ Only Static Spaces are truly free. Creating new Gradio/Docker/Streamlit Spaces on cpu-basic requires a PRO subscription. Existing pre-PRO Spaces continue working. |
| `cpu-upgrade` | 8 | 32 GB | 50 GB | $0.03/hr | Requires PRO+ |
...
- **Requires PRO subscription** — `create_repo(space_sdk="gradio")` returns 402 Payment Required on free accounts. The hardware tier being `cpu-basic` does NOT make it free; creation itself is behind the paywall.
- ZeroGPU (`zero-a10g`) is available for PRO users as a free GPU tier (limited daily quota)
...
## Related References
- `references/live-data-static-spaces.md` — Live-data dashboards with static HTML + JS (Gradio alternative for free tier), HF API fetching patterns, git deploy workaround for `hf upload` 402 failures
- `references/hf-learnings.md` — Deep-dive notes on Spaces configuration
