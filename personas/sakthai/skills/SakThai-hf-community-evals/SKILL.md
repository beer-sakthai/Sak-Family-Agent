...
description: "Complete reference on HF Community Evals (decentralized eval results on model pages\
  \ and benchmark leaderboards) and its cross-compatibility with Every Eval Ever (EEE)\
  \ \u2014 the EvalEval Coalition's universal eval schema. Covers eval.yaml spec,\
  \ .eval_results/ YAML format, community PR submission workflow, benchmark registration,\
  \ and the EEE converter tool for cross-posting results with verified source badges."
---
