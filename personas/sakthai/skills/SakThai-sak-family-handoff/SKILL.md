---
name: SakThai-sak-family-handoff
author: SakThai
license: MIT
...
| `"every 5m"`, `"30m"`, `"2h"` | ✅ With repeat | ✅ Duration-based recurring |
| `"0 9 * * *"` | ✅ Runs N days | ✅ Daily at 9AM |

**Key nuance:** Even though the system *displays* your schedule as `"once in 1m"` after creation, you MUST submit it as `"1m"` or `"every 1m"`. The `"once in"` display is internal normalization. For finite series (N runs), ALWAYS use `"every <N>m"` + `repeat=N`. For indefinite recurring, omit `repeat`.

**Diagnosis:** If job shows `repeat: 1/N`, `next_run_at: null`, `state: completed` → schedule was interpreted as one-shot. Remove and re-create with `"every 1m"` + `repeat=N` (or self-limit inside the prompt with `"every 1m"` + `"forever"`).
