...
from evolution.core.fitness import skill_fitness_metric, FitnessScore, _parse_score, LLMJudge
...
class TestFitnessScore:
    def test_composite_score(self):
        ...
        score = FitnessScore(correctness=1.0, procedure_following=0.0, conciseness=0.0, length_penalty=0.1)
        # 0.5*1.0 + 0 - 0.1 = 0.4
...
class TestLLMJudge:
    ...
    def test_score_basic(self, judge):
        ...
        with patch.object(judge, 'judge', return_value=mock_result):
    ...
    def test_length_penalty(self, judge):
        ...
        with patch.object(judge, 'judge', return_value=mock_result):
            with patch('dspy.LM'), patch('dspy.context'):
                # 95% size
                score = judge.score(
                    task_input="i", expected_behavior="e", agent_output="o", skill_text="s",
                    artifact_size=95, max_size=100
                )
                # ratio = 0.95
                ...
                score = judge.score(
                    task_input="i", expected_behavior="e", agent_output="o", skill_text="s",
                    artifact_size=110, max_size=100
                )
                # ratio = 1.1
